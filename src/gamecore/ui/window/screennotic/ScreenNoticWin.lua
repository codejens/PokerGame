...
----跑马灯
------------------------------
ScreenNoticWin = simple_class(GUIWindow)
------------------------------
...
------------------------------
local _screen_notic_default_begin_pos_x = 500
local _screen_notic_default_begin_pos_y = 300
...
local _screen_notic_default_wait_time = 0.25
------------------------------
local _default_font_size = 19
local _default_timer_stept = 0.002
------------------------------
local _run_self_timer = false
local _self_timer_rate = 0.05 + 0.2
...
function ScreenNoticItem:__init(info, fontSize)
	...
	local x_border = 50
	local y_border = 4
	local label = CCZXLabel:labelWithText( x_border, 39/2, temp_info, fontSize, ALIGN_LEFT )
	...
	if type(_screen_notic_data) == 'table' and #_screen_notic_data > 0 then
		...
		local function timer_fun(eventType, arg, msgid, selfItem)
			...
			if eventType == TIMER then
				...
				temp_item.view:setOpacity( notic_item.cur_alpha ) 
			end
		end
...
function ScreenNoticWin:create_notic(tinfo, tfontSize, tx, ty, anchor)
	local is_true = ScreenNoticWin:add_notic(tinfo, tfontSize, tx, ty, anchor)
...
function ScreenNoticWin:add_notic(tinfo, tFontSize, tx, ty, anchor)
	if anchor then
		tx, ty = _calculateScreenPos(tx, ty, anchor)
	end
	local temp_info = {x = tx, y = ty, info = tinfo, fontSize = tfontSize}
	local num = #_screen_notic_data
	if num > 0 then
		-- if _screen_notic_data[num].info == tinfo then
		-- 	return false 
		-- end
		-- if num > 20 then
			-- table.remove(_screen_notic_data, 1)
