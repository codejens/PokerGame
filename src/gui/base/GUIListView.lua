-----------------------------------------------------------------------------
-- 滚动控件
-- @author zengsi
-- @release 1
-----------------------------------------------------------------------------

GUIListView = simple_class(GUITouchBase)

function GUIListView:__init( view )
	self.class_name = "GUIListView"
	self:setTouchEnabled(false)
end

--创建图片控件函数
--@param texture 贴图资源
function GUIListView:create(texture)
	if texture then
		return self(ccui.ImageView:create(texture))
	else
		return self(ccui.ImageView:create())
	end
end

function GUIListView:create_by_layout(layout)
	local img = self(self:create(layout.img_n).core)
	img:setPosition(layout.pos[1],layout.pos[2])
	if layout.size then
		img:setContentSize(layout.size[1],layout.size[2])
	end
	if img.is_nine then
		img:setScale9Enabled(true)
	end
	return img
end

--创建九宫格图片函数
--@param texture 图片路径
--@param rect  拉伸CapInsets
function GUIListView:create9Img( texture,rect )
	local img = self(self:create(texture).core)
	img:setScale9Enabled(true)
	img:setCapInsets(rect or _rect)
	return img
end

--设置贴图函数
--@param texture 贴图资源
function GUIListView:loadTexture( texture )
	self.core:loadTexture(texture)
end

--启用九宫格
function GUIListView:setScale9Enabled( enabled )
	self.core:setScale9Enabled(enabled)
end
--设置九宫格rect
function GUIListView:setCapInsets( rect )
	self.core:setCapInsets(rect)
end

