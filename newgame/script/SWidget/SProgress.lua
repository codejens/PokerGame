...
function SProgress:create_by_layout(layout)
	...
	--进度值
	local progress = CCBasePanel:panelWithFile(0,0,layout.size[1],-1,layout.img_n,500,500)
	mask_panel:addChild(progress)
	local s = progress:getContentSize()
	progress:setSize(layout.size[1],layout.size[2])
	...
	obj:set_progress_size(layout.size[1],s.height)
	return obj
end

--设置当前值,默认 10/100
--要显示百分比,就先改变style样式
function SProgress:set_value(cur_percentage, max_percentage,param)
	...
	if self.style_label == 2 then
		param = param or ""
		self:setText(self.color .. cur_percentage .. "/" .. max_percentage .. param)
	else
...
function SProgress:set_progress_size(w, h, offset_x, offset_y)
	...
	self.progress:setPosition(self.progress_pos.x,self.progress_pos.y )
