...
function SDragBar:create_by_layout(layout)
	...
	local min_move_img  = CCZXImage:imageWithFile(2, 2, but_current_x, height-4, bar_image, 500, 500)
	value_move_bar.view:addChild(min_move_img)
	...
	local function update_layout()
		...
		if but_current_x > min_can_move_var then
			min_move_img:setSize(min_can_move_var, height-4)
			min_move_img:setPosition(btn_dis+2, (base_h-height)/2+2)
		else
			min_move_img:setSize(but_current_x, height-4)
		    min_move_img:setPosition(btn_dis+2, (base_h-height)/2+2)
		end
		if min_can_move_var <=1 then
		   min_move_img:setIsVisible(false) 
		else
		   min_move_img:setIsVisible(true) 
		end
	end
	update_layout()
	--计算当前值
	...
	local function set_but_position(pos_x)
        ...
        if pos_x<= min_can_move_var then
        	pos_x = min_can_move_var
        end
	...
	local function touch_event_func(eventType, touch_x_y)
		if eventType == TOUCH_BEGAN and not self.touch_all then
			...
			local node = value_move_bar.view
			node:setHittestType(2)
			local node_parent = node:getParent()
			while (node_parent and node_parent.setHittestType) do
				node_parent:setHittestType(2)
				node = node_parent
		...
		if (eventType == TOUCH_CANCEL or eventType == TOUCH_ENDED) and self.touch_all then
			...
			local node = value_move_bar.view
			local node_parent = node:getParent()
			node:setHittestType(0)
			while (node_parent and node_parent.setHittestType) do
				node_parent:setHittestType(0)
				node = node_parent
				node_parent = node:getParent()
			end
		end
	...
	value_move_bar.set_current_value = function(curr_value)
		local curr_pox_x = (curr_value/max_value)*(base_w-but_image_w)
	...
	--设置最低可滑动的进度条
	value_move_bar.setMinMoveTexture = function(file)
		min_move_img:setTexture(file)
	end
	--按钮纹理
	...
	value_move_bar.set_min_canMove = function(value)
         min_can_move_var = (value/max_value)*(base_w-but_image_w)
	end
...
--设置最小的可移动位置 xiehande
function SDragBar:set_min_canMove( value )
	if self.dragbar.set_min_canMove then
		self.dragbar.set_min_canMove(value)
	end
end

--设置不能移动位置的图集xiehande
function SDragBar:setMinMoveTexture( value )
	if self.dragbar.setMinMoveTexture then
		self.dragbar.setMinMoveTexture(value)
	end
end