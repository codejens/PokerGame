...
function SelectRoleCC:get_role_time( id )
	for i=1,#_role_date do
		if _role_date[i].id == id then
			return _role_date[i].createTime or 0
		end
	end
	return 0
end
...
function SelectRoleCC:do_result_create_role( pack )
	...
	local create_time 	= pack:readUInt()
	print("ZXLuaUtils:GetTickCounts()",ZXLuaUtils:GetTickCounts(),create_time)
	_role_date  ={}
	_role_date[1] = {id =role_id,createTime =create_time  }
	-- if error_id == 0 then
		-- 角色成功, 请求进入游戏场景
		require "model/RoleModel"
		-- RoleModel:enter_game_scene( role_id, error_id )
		-- print("返回创建角色结果s->c 255,2")
...
function SelectRoleCC:do_result_enter_game( pack )
	...
	local error_id = pack:readByte()
	-- print("返回进入游戏的结果c->s 255,5",error_id)
	if error_id == 0 then
		--  成功, 
		require "model/RoleModel"
...
function SelectRoleCC:do_exit(pack)
	...
	-- enIsOnline =3,       //已经在线,注意：如果已经在线，客户端应该在1秒后重试登录
	-- enServerBusy =4,     //服务器忙
...
--登录服心跳
function SelectRoleCC:second_tick()
	if NetManager:get_socket() then
		local pack = NetManager:get_socket():alloc(255, 12)
		pack:writeInt(os.time())
		NetManager:get_socket():SendToSrv(pack)	
	end
end