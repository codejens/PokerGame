...
function BISystem:send_log(url, param)
	if _log_server_address == nil or _log_server_address == "" or _log_server_address == "1" or url == nil then
		return
	end
...
--发送打点日志 长歌行
function BISystem:send_cgx_log(stype,uid,role_id,role_name,serverId)
	if _log_server_address == nil or _log_server_address == "" or _log_server_address == "1" then
		return
	end

	local udid = ""
	local _uid = uid or ""
	local _role_id = role_id or ""
	local _role_name = role_name or ""
	local _type = stype or ""
	local platform = BISystem:getDownloadFrom() or ""
	local idfa = ""
	local server_id = serverId or ""
	...
	HttpRequest:sendBI(_log_server_address, param, BI_log_response, false)
end
