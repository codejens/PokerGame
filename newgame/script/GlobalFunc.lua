...
--print("function GlobalFunc:teleport( target_scene_id,npc_name )")
	...
	-- printc("速传的数据==========>>>>>>>>>", target_scene_id, npc_name, 13)
	-- 设置传送过去后的动作
	...
	local scene_x,scene_y,offeset = SceneConfig:get_npc_pos( target_scene_id , npc_name );		
	--MiscCC:req_teleport(target_scene_id,scene_name,scene_x,scene_y);

	-- print (">>>>>>>>>>>>>> GlobalFunc:teleport >>>>>>>>>>>>", scene_x, scene_y, SceneManager:can_move(scene_x,scene_y))

	-- local function conFun(x,y)		
	-- 	return SceneManager:can_move(x,y)
	-- end

	-- local x,y = Utils:spiral_scan(4, 3, scene_x, scene_y, conFun)
	MiscCC:req_teleport(target_scene_id,scene_name,scene_x+offeset[1],scene_y+offeset[2]);
end
...
-- 允许寻路的副本，副本id
-- local ai_allowmove = {
-- 	SFuBenModel.FB_LLHUANJING_ID,
-- }
-- function GlobalFunc:ai_allow_tomove( fb_id)
-- 	for k,v in pairs(ai_allowmove) do
-- 		if fb_id == v then
-- 			return true
-- 		end
-- 	end
...
function GlobalFunc:move_to_target_scene( target_scene_id,x,y ) 
	-- print( "-----target_scene_id ~= SceneManager:get_cur_scene() and SceneManager:get_cur_fuben() > 0 :", target_scene_id, SceneManager:get_cur_scene())
	-- print( target_scene_id ~= SceneManager:get_cur_scene() , SceneManager:get_cur_fuben() > 0)
	local cur_fbid = SceneManager:get_cur_fuben()
	if target_scene_id ~= SceneManager:get_cur_scene() and cur_fbid > 0 and not self:ai_allow_tomove(cur_fbid) then
		GlobalFunc:create_screen_notic(LangCommonString[18]) -- [18]="副本中不能自动寻路"
		return
	end
...
function GlobalFunc:teleport_to_target_scene( target_scene_id,x,y, npc_name ) 
	...
	local scene_name = SceneConfig:get_scene_name_by_id(target_scene_id);	
	-- print("1x,y=",x,y)
	-- x,y = SceneManager:pixels_to_tile(x,y)
	--MiscCC:req_teleport(target_scene_id,scene_name,x,y);
	-- print("2x,y=",x,y)

	-- local function conFun(tx,ty)
	-- 	if tx ~= x and ty ~= y then
	-- 		return SceneManager:can_move(tx, ty)
	-- 	end
	-- end
	if npc_name then
		AIManager:set_after_pos_change_command( target_scene_id , AIConfig.COMMAND_ASK_NPC, npc_name  );
	end
	-- local scan_x, scan_y = Utils:spiral_scan(3, 2, x, y, conFun)
	MiscCC:req_teleport(target_scene_id,scene_name,x,y);
end
...
--						27,打开宠物界面,28,坐骑引导界面,29,渡劫30,灵根31,演武场32,兑换
