...
function _timer:start(_time, _function,call_func)
	local t = reserve
--@debug_begin
...
--@debug_end
	...
	--add by tjh 加个安全处理防止time函数报错无法停止 过多导致崩溃
	self.tick = true
	local function time_func( dt )
		...
		_function(dt,call_func)

		local e = ZXLuaUtils:GetTickCounts()
		luaPerformance:add_performance(string.format("%s  _timer性能检查",tostring(_function)),e-s)
	
		self.tick = true
	end
	self.scheduler_id = scheduleScriptFunc(CSS,time_func,_time,false)
end
