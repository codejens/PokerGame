--MainWin.lua
super_class.GameWin(BaseEditWin)
...
function GameWin:create_makers()
	if self.define_makers_pos == nil then
		self:init()
	end
	local array = {}
    array.id = 1000
...
function GameWin:create_player(player_info)
	local layout = {
		img_n = "sui/common/gold_home_avatar.png",
	...
	print("player_info.id=",player_info.id)
	if RoleModel:is_self(player_info.id) then
		name = name .. "(自己)"
	end
	local label_name = SLabel:create(name)
	panel_player:addChild(label_name)
	label_name:setPosition(0,73)
	
	--货币
	local label_money = SLabel:create(player_info.money)
...
function GameWin:delete_player(index)
	...
	if panel_player then
		panel_player:removeFromParentAndCleanup(true)
		panel_player = nil
