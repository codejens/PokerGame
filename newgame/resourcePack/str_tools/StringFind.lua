...
-------------------------add by hcl on 2014/3/10-------------
function parse_game_str( file )
	filename = file
	local mainfile = io.open(filename,'r')
	local c = mainfile:read('*all')
	c = string.gsub(c,'[\r]','\n')
	...
	local newfilecontent = {}
	for line in f:lines() do
		local start_i,l_end_i = string.find(line,lanfilename);
		-- print("start_i",start_i,line)
		if start_i~= nil then
			print("line = ",line,l_end_i)
			-- 截短的LangGameString索引字符串
			local sub_str = string.sub(line,l_end_i);
			-- 找到的LangGameString索引右中括号的索引			 
			local end_i = string.find(sub_str,"%]");
			-- LangGameString索引 
			local index_str = string.sub(sub_str,2,end_i);
			print("index_str",index_str);
			-- 截取注释中的原中文
			local ignore_str = string.sub(sub_str,end_i + 1)
			print("ignore_str=",ignore_str)
			local ignore_i = string.find(ignore_str,index_str);
			if ignore_i then
				print("ignore_i",ignore_i)
				local ignore_str_start_i = string.find(ignore_str,"\"");
				local ignore_str_end_i = string.find(ignore_str,"\"",ignore_str_start_i+1);
				local str = string.sub(ignore_str,ignore_str_start_i,ignore_str_end_i);
				local rep_str = "LangGameString"..index_str
				print("str=",str,rep_str)
				local r_str = string.gsub(line,rep_str,str )
				print("r_str=",r_str)
				-- line = string.sub( line,1,ignore_i-1 );
				-- print("line=",line)
			end
		end
		newfilecontent[#newfilecontent+1] = line
...
-- print('findFile:',findFile,'findMatch',findMatch)
--print()
