...
local binaryPath = "binary" 	--bin文件所在目录名
local scriptType = ".lua"	 	--脚本扩展名
local binaryType = ".bin"   	--bin文件扩展名
local RecordRevistionFile = ".\\binary\\addComplie.version" --增量打包版本号记录文件
local SVNLogTemp = "C:\\svnLog.txt" --保存script差异信息
...
function GetNewRevision()
	...
	os.execute("resourcePack\\bin\\svn info " .. scriptPath .. " > C:\\svnTemp.txt" )
	local svnTempFile = io.open("C:\\svnTemp.txt", "r")
	if svnTempFile then
...
function ComplieFileAndMove(proJectPath, relativePath)
	if string.find(relativePath, ".lua$") then
		...
		local cmdEncrypt = batWorkPath .. "\\resourcePack\\Encrypt " .. luacFile .. ' ' .. binaryType
...
function GetCommitMsg()
	os.execute("resourcePack\\bin\\svn log -l 1 " .. scriptPath .. " > C:\\svnCommitLog.txt")
	local file = io.open("C:\\svnCommitLog.txt", "r")
...
if ComplieFile() then
	if binaryChange  then
		RecordNewRersion()
		os.execute("resourcePack\\bin\\svn log -l 1 " .. scriptPath .. " > C:\\svnCommitLog.txt")
		os.execute("resourcePack\\bin\\svn commit -F " .. " C:\\svnCommitLog.txt "   .. binaryPath)
	else
