...
function DoPack(reset)
	...
	ret,  pPackType, pFileType, pWidth, pHeight,pSort,pInputPath,--[[pOutputPath,]]pAlpha, pAnchorX,pAnchorY,pCompress,pWebp =
		  iup.GetParam("图片打包工具", param_action,
					  ...
					  --[[pOutputPath,]] pAlpha,
					  pAnchorX,pAnchorY,pCompress,pWebp)
	if (not ret) then
	  return false
	end
	...
	elseif outprefix == '' then
		...
		local cmd = string.format('lua framepacker.lua %d %d %d %s %d %d %s %s %s %s %d %d',
					pAnchorX,
					pAnchorY,
					pAlpha * 10000,
					file_type[pFileType + 1],
