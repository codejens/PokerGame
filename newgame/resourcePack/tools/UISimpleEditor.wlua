...
console.savebtn = iup.button{ title = "保存到layouts" }
...
console.absPosbtn = iup.button{ title = "修改" }
...
console.texturebtn = iup.button{ title = "修改" }
...
console.dialog = iup.dialog
{
  iup.vbox
  {
	...
	iup.frame
    {
		...
		{
			iup.label{title = '窗口类型(根节点)'},
			console.root_class,
			iup.label{title = 'tag'},
			console.tag,
			iup.hbox
			{
				iup.label{title = '控件C++类型'},
				console.comp_name,

			},
			iup.hbox
			{
				iup.label{title = '控件脚本类型'},
				console.comp_class,
			}
		},
		title = "类型:",
...
function setSelected(tag,x,y,zOrder,absX,absY,width,height)
	--print(tag,x,y)
	console.tag.value = tag
	console.posX.value = x
	console.posY.value = y
  console.zOrder.value = zOrder
	console.absPosX.value = absX
	console.absPosY.value = absY
...
function refreshGUI(dict)
	...
	local x 	= dict.rect[1]
	local y 	= dict.rect[2]
	local absX 	= dict.rect[3]
	local absY 	= dict.rect[4]
	local width = dict.rect[5]
	local height = dict.rect[6]
	setSelected(dict.tag, x, y, absX, absY, width, height)
	setCompClasses(dict.root_class,dict.comp_class)
	setTexture(dict.texture)
	setName(dict.comp_id)
...
function console.posbtn:action()
	srv:Send(string.format('UIEditor:setSelectedPosition(%d,%d,%d,%d)',
	console.posX.value,
	console.posY.value,
	console.absPosX.value,
	console.absPosY.value))
end
...
function console.texturebtn:action()
	srv:Send(string.format('UIEditor:setSelectedTexture(%q)',console.texture.value))
end

function  console.savebtn:action()
	srv:Send('UIEditor:save()')
end
