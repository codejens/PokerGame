...
function DoPack(reset)
	...
	elseif outprefix == '' then
		...
		print("-xx-",deleteP.. '*'.. file_type[pFileType + 1] )
		print("-xx-",deleteP .. '*.frame')
		...
		--plist自动转为frame 并修改后缀
		local path = "..\\resource\\"..pOutputPath..'\\'..pOutputFilePrefix..'1.plist'
		print("plist-path=",path)
		plistTool.convert(path)
		os.execute('del /s ' .. deleteP .. '*.plist')
		os.execute('del /s ' .. deleteP .. '*.pd')
		os.execute('ren ' ..deleteP..'*.png *.pd')
