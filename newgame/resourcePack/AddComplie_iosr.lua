...
require 'resourcePack/packCode/ImageSetReader'

local scriptPath = "resource" 	--脚本所在目录名
local binaryPath = ".\\AppleStoreRelease\\resource" 	--bin文件所在目录名
local scriptType = "%.lua"	 	--脚本扩展名
local binaryType = "%.nst"   	--bin文件扩展名
local RecordRevistionFile = ".\\AppleStoreRelease\\addComplie1.version" --增量打包版本号记录文件
...
function GetFileName( path,stype )
	local t = Split(path,"\\")
	t = t[#t]
	t = Split(t,stype)
	t = t[1]
...
function ComplieFileAndMove(proJectPath, relativePath)
	...
	--直接拷贝过去
	local src_path = proJectPath..relativePath
	local dest_path = proJectPath.."AppleStoreRelease\\"..relativePath
	local cmdCopy = "copy /Y " .. src_path .." ".. dest_path
	os.execute(cmdCopy)
	--资源压缩
	if string.find(relativePath, "frame\\effect") 
		or string.find(relativePath, "frame\\buffeffect") 
		or string.find(relativePath, "frame\\uieffect")  
		or string.find(relativePath, "frame\\weapon") then
		print("relativePath11===========",relativePath)
		
		--压缩处理
		if string.find(relativePath, ".pd") then
			local name = GetFileName(relativePath,".pd")
			local _path = Split(relativePath,name..".pd")
			_path = _path[1]

			--重命名
			local cmdRen = "ren "..dest_path.." "..name..".png"
			os.execute(cmdRen)
			
			--压缩
			local _src_path = proJectPath.."AppleStoreRelease\\".._path..name..".png"
			local _des_path = proJectPath.."AppleStoreRelease\\".._path..name..".pvr.gz"
			local _des_path1 = proJectPath.._path..name..".plist"
			local cmdTex = "resourcePack\\TexturePacker\\bin\\TexturePacker.exe "
			cmdTex = cmdTex .._src_path .. " --sheet ".._des_path .. " --data ".._des_path1
			cmdTex = cmdTex .." --texture-format pvr2gz --opt PVRTC4 --format cocos2d --trim-mode None --border-padding 0 --shape-padding 0 --inner-padding 0  --extrude 0 --force-squared"
			os.execute(cmdTex)

			--删除
			local cmdDel = "del ".._src_path
			os.execute(cmdDel)
			--删除
			local cmdDel = "del ".._des_path1
			os.execute(cmdDel)

		elseif string.find(relativePath, ".frame") then
...
--先增加目录
function AddFileDir(proJectPath, relativePath )
	relativePath = string.gsub(relativePath, "/", "\\")
	local fullPath = proJectPath .. relativePath
	local info = lfs.attributes(fullPath)
...
function ComplieFile()
	...
	while line do
		local Scs, ScE = string.find(line, "resource") --使用路径中的script确定操作的是script目录中的文件
		if Scs then
			if not string.find(line, "resourceTree") and
			 not string.find(line, "resourcePack") then
	...
	for i, v in ipairs(t_AddFile) do
		if AddFileDir(projectPath, v) == false then
...
local pvrpath = {
	batWorkPath..binaryPath.."\\frame\\effect",
	batWorkPath..binaryPath.."\\frame\\uieffect",
	batWorkPath..binaryPath.."\\frame\\weapon",
	batWorkPath..binaryPath.."\\frame\\buffeffect",
}
...
for i=1,#pvrpath do
	local textureEdit = {}
	ImageSetReader.patchLoad_pvr(pvrpath[i],textureEdit)
end
