...
--[[
	...
	2)药水类物品的配置，是buff药才需要这样配置,物品类型= 103，(可以多个buff，比如2个buff，下面的要配置8行,...)
		...
		desc = LangGameString[101],  -- [101]="物品描述（说明）文字，可使用HTML语法"
	},
...
ItemConfig.ITEM_TYPE_MARRIAGE_RING 		= 11 	-- 婚戒
...
ItemConfig.ITEM_TYPE_GEM				= 202	-- 宝石
...
ItemConfig.GEM_GOUL_TIP					= 1004	-- 法宝器魂的tip
...
ItemConfig.SHENSHOU_CIFU_TIP			= 1009	-- 显示神兽赐福的tip
ItemConfig.SPECIAL_MOUNT_TIP			= 1010  -- 显示特殊坐骑的tip
ItemConfig.SIMPLE_TIP					= 1011  -- 简单tip，只有一段dialog(挖矿用到)
ItemConfig.MEIREN_TIP					= 1012  -- 美人卡牌tip
...
function ItemConfig:get_item_by_id( item_id )
	...
	if item == nil then
		...
		item = ItemModel:get_temp_item_info( item_id );
		-- print("临时道具数据里面的item",item_id,item);
	end
	return item
end
...
function ItemConfig:get_item_icon( item_id )
	...
	-- 暂时先
	-- return icon_path .. "test_bag.png"
	-- return "ui/test/yifu.png"
	-- TODO:: 这里需要检查一下是否存在这个图标
	return string.format("%s%05d.pd", icon_path, item.icon)
	--return string.format("%s%05d.png", icon_path, item.icon)
end
...
-- 是否是物防宝石
function ItemConfig:is_phy_prot_gem( item_id )
	if item_id == nil then
        return nil
	end
	--防御宝石的id集合，用来判断某件物品是否是防御宝石
    local _prot_gem_num_t = { [18520] = true, [18521] = true, [18522] = true, [18523] = true,
                    [18524] = true, [18525] = true, [18526] = true, [18527] = true,
                    [18528] = true, [18529] = true}
    return _prot_gem_num_t[ item_id ]   
end

-- 是否是法防宝石
function ItemConfig:is_mag_prot_gem( item_id )
	if item_id == nil then
        return nil
	end
	--防御宝石的id集合，用来判断某件物品是否是防御宝石
    local _prot_gem_num_t = { [18530] = true, [18531] = true,
                    [18532] = true, [18533] = true, [18534] = true, [18535] = true,
                    [18536] = true, [18537] = true, [18538] = true, [18539] = true}
    return _prot_gem_num_t[ item_id ]   
end
...
function ItemConfig:get_gem_type_by_id( gem_item_id )
    if ItemConfig:is_attack_gem(gem_item_id) then
        return 1
    elseif ItemConfig:is_phy_prot_gem(gem_item_id) then
        return 2
    elseif ItemConfig:is_mag_prot_gem(gem_item_id) then
        return 3
    elseif ItemConfig:is_life_gem(gem_item_id) then
        return 4
    else
        return nil
    end
...
function ItemConfig:get_equip_quality_name_by_quality( quality )
	if quality == 1 then
		return LangGameString[102]; -- [102]="[普通]"
	elseif quality == 2 then
		return LangGameString[103]; -- [103]="[优秀]"
	elseif quality == 3 then
		return LangGameString[104] -- [104]="[精良]"
	elseif quality == 4 then
		return LangGameString[105] -- [105]="[完美]"
	elseif quality == 5 then
		return LangGameString[106] -- [106]="[神圣]"
	end
end
