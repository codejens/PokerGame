...
--[[
{
			...
			{
				{
					...
					type：特效的类型,无效果=0,挥洒=1,中心爆炸=2,飞行=3,脚下爆炸=4,脚下持续=5,持续=6,中心爆炸(攻击者朝向)=7
					id:特效的ID
					keepTime:持续时间,单位是毫秒
	...
	24:需要阵营的职位,value表示职位的id
		1=LangGameString[340] 2=LangGameString[341],  3=LangGameString[342], 4=LangGameString[343],  5=LangGameString[344], 6=LangGameString[345], 7=LangGameString[346],  -- [340]="盟主" -- [341]="奉天盟使" -- [342]="承坤盟使" -- [343]="冥苍护法" -- [344]="逸白护法" -- [345]="炎朱护法" -- [346]="铁玄护法"
		8=LangGameString[347],9=LangGameString[348],10=LangGameString[349],11=LangGameString[350],12=LangGameString[351],13=LangGameString[352],14=LangGameString[353],15=LangGameString[354], -- [347]="蟠云卫" -- [348]="坎雷卫" -- [349]="玉风卫" -- [350]="啸电卫" -- [351]="南明卫" -- [352]="璃火卫" -- [353]="紫川卫" -- [354]="灵武卫"
...
SkillConfig.SKILL_TYPE_COMBAT_ATTACK = 7 				--7普通攻击
...
--持续=6,
--中心爆炸使用施法者朝向=7
--技能施放以主角为基准点，方向为主角方向=8，天降雄师新增。add by gzn
--受击特效 -- 9
...
SKILL_EFFECT_TYPE_ONCE_FROM_SRC = 8
SKILL_EFFECT_TYPE_BE_ATTACKED = 9

--施法失败， 公共CD中
SkillConfig.USE_SKILL_COMMOM_CD_FAILED = 2
local SKILL_ENABLE = 
{ 
	[1] = true,
	[2] = true,
...
require '../data/action_effects/body_skill_config'
...
function SkillConfig:get_skill_icon( skill_id )	
	...
	if skill == nil then
		...
		return icon_path .. "default.pd"
		--return icon_path .. "default.png"
	end
	-- 暂时先
	-- return icon_path .. "default.png"
	return string.format("%s%05d.pd", icon_path, skill.skillSubLevel[1].iconID)
	--return string.format("%s%05d.png", icon_path, skill.skillSubLevel[1].iconID)
end
...
function SkillConfig:get_skill_icon_path( skill_id,skill_lv)
	...
	local path = "icon/skill/00" .. icon_id .. ".pd";
	--local path = "icon/skill/00" .. icon_id .. ".png";
	return path;
end
...
function SkillConfig:get_skills_by_job( job_type )
	local skill_t = {}
	local job_to_skill_id = { [1] = { 1, 3, 2, 4, 5, 6 },
	                          [2] = { 9, 12,11, 14, 15, 16 },
	                          [3] = { 17,20, 19, 22, 23, 24 },
	                          [4] = { 25,28, 27, 30, 31, 32 }}
...
function SkillConfig:get_spell_action( id, level )
	...
	local skill  	= load_skill_config(id)
	local entity 	= skill.skillSubLevel[level]		-- 技能每个等级的配置
	local action    = spell_action_effect_config[id]
	assert(action,string.format('没有配置技能id = %d %s',id,skill.name))
	if entity.actions ~= nil then
		-- 虽然配置可能有很多动作序列，但我们目前只用第一个
		local action_id	= action.act
		local effect_id	= action.effect
		-- 动作id需要转换一下
		-- 现在暂时0动作1,1和2动作2
...
function SkillConfig:get_spell_action( id, level )
	--xprint('')
	--local _isCombatAttack = SkillConfig:isCombatAttack(id)

	--[[
	if actionTable then
		local entry_id = math.random(1,#actionTable)
		local entry = actionTable[entry_id]
		return entry_id.act, entry_id.effect
	end
	]]--
	...
	--print('skill',skill.name, skill.skillType)

	if skill.actions ~= nil then
		local entry_id = math.random(1,#skill.actions)
		local action  = skill.actions[entry_id]
		print("---------action.act:", action.act, action.effect, action.type)
		print("---------action.act:", action.act, action.effect, action.type)
		print("---------action.act:", action.act, action.effect, action.type)
		return action.act, action.effect, action.type -- "type":add after tjxs
	else
--@debug_begin
	print(string.format('没有配置技能id = %d %s',id,skill.name))
--@debug_end
		print("-----------AX_ACTION_HIT:", ZX_ACTION_HIT, 0)
		return ZX_ACTION_HIT, 0
	end
...
function SkillConfig.isCombatAttack(skill_id)
	local skill = load_skill_config(skill_id)
	return skill.skillType == SkillConfig.SKILL_TYPE_COMBAT_ATTACK
end

--[[
function SkillConfig.isCombatAttack(skill_id)
	return normal_attack_action_effect_config[skill_id]
end
]]
...
-- 获取技能在某个等级的desc
function SkillConfig:get_skill_by_id_and_level(skill_id,skill_level)
	local skill = load_skill_config(skill_id)
	local skill_name = skill.name
	local skill_desc = skill.skillSubLevel[skill_level].desc
	return skill_name,skill_desc
end

function SkillConfig.isSkillEnable(skill_id)
	return SKILL_ENABLE[skill_id]
end



function SkillConfig.isSkillCanbeClick(skill_id)
	local skill = load_skill_config(skill_id)
	return SKILL_CAN_CLICK[skill.skillType]
end


function SkillConfig.isPassiveSkill(skillType)
	return SKILL_PASSIVE[skillType]
end

function SkillConfig:isPassiveSkillById( skill_id )
	local skill = SkillConfig:get_skill_by_id(skill_id)
	return SkillConfig.isPassiveSkill(skill.skillType)
end

-- 此处的body_id实为job职业(1234)
function SkillConfig.getSimpleAttackWithBodyID(body_id, skill_id)
	print("SkillConfig.getSimpleAttackWithBodyID body_id,skill_id",body_id,skill_id)
	local dict = body_normal_attack_config[body_id]
	if not dict then
		return body_normal_attack_config.default_normal_attack
	else
--@debug_begin
		assert(dict, body_id)
		if dict[skill_id] == nil then
			print('无法找到普通攻击技能 body_id', body_id, 'skill_id', skill_id )
...
--@debug_end
		print("dict[skill_id]",dict[skill_id])
		return dict[skill_id]
	end
end

function SkillConfig:get_super_skill()
	local skill_t = TransformConfig:get_super_skill(  )
	local super_skill_t = {}
	for k,v in pairs(skill_t) do
		super_skill_t[v] = true
		-- print("===========: ", v, super_skill_t[v])
	end
