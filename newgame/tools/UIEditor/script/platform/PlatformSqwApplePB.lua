-- PlatformSqwApplePB.lua
-- created by guozhinan on 2015-1-5
-- 天将雄师项目37wan的越狱(prison break)版ios平台

PlatformSqwApplePB= {}
PlatformSqwApplePB.MESSAGE_TYPE	= "platform"
PlatformSqwApplePB.FUNC_CHANGE_ACCOUNT = "change_account"
PlatformSqwApplePB.FUNC_LOGIN		= "login"		--登录
PlatformSqwApplePB.FUNC_PAY		= "pay"			--支付
PlatformSqwApplePB.FUNC_TAB		= "tab"			--切换账号
PlatformSqwApplePB.FUNC_LOGIN_OUT	= "login_out"	--登出
PlatformSqwApplePB.FUNC_INIT_ROLE	= "init_role"	--传送

PlatformSqwApplePB.CODE_SUCCESS = 0
PlatformSqwApplePB.STATE_LOGIN_SUCCESS = 0 -- 成功
PlatformSqwApplePB.STATE_LOGIN_FAIL = 1	-- 失败

-- 不准许有全局参数，因为需要被reload
--
--
function PlatformSqwApplePB:init()
...
-- */
function PlatformSqwApplePB:init_role_info()
	-- ios版好像没有这个需求
	...
	-- json_table_temp[ "message_type" ]	= PlatformSqwApplePB.MESSAGE_TYPE	--消息类型，必传字段
	-- json_table_temp[ "function_type" ] = PlatformSqwApplePB.FUNC_INIT_ROLE
...
-- 入口,当Login界面Show
-- 显示登陆界面
function PlatformSqwApplePB:onEnterLoginState()
	...
	if self.login_state == false then
		...
		local c = callback:new()
			c:start(0.1,function()
				PlatformSqwApplePB:doLogin()
			end)
	end
end

-- 按下登录按钮的回调，应显示平台登陆页
function PlatformSqwApplePB:doLogin()
	--跳转到无平台登陆页，有平台应该调用SDK
	-- RoleModel:change_login_page( "login" )
	-- MUtils:toast("PlatformSqwApplePB:doLogin", 2048, 3)
	-- RoleModel:change_login_page( "login" )
	...
	if self.firstLogin == true then
		...
		local json = "{\"funcID\":\"sdk_sqwpb_switch_account\"}"
	...
	local function login_succ( error_code, json )
		...
		if error_code == 0 then
			...
			-- if PlatformSqwApplePB.firstLogin == true then
				local access_token = json_table["token"]
				PlatformSqwApplePB:set_access_token(access_token)
				-- pid是37wanSDK登录回调给的联运id号，暂时不知道有什么用，先保存再说
				self.pid = json_table["pid"]
				-- self.firstLogin = false
				RoleModel:request_server_list_platform()
			-- elseif PlatformSqwApplePB.firstLogin == false then
	...
	local json = "{\"funcID\":\"sdk_sqwpb_login\"}"
	IOSDispatcher:async_send_to_oc(json,login_succ)
	-- IOSDispatcher:sdksqw_logout()
end

--获取平台登录url
function PlatformSqwApplePB:get_login_url()
...
--获取服务器列表url
function PlatformSqwApplePB:get_servlist_url()
	return GameUrl_global:getServerIP() or ""
end

function PlatformSqwApplePB:show_logout(callbackfunc)
	callbackfunc()
end


--从选服界面登出
function PlatformSqwApplePB:logoutFromSelectServer(callbackfunc, reason)
...
-- function PlatformSqwApplePB:get_servlist_param()
...
-- login 的参数
function PlatformSqwApplePB:get_login_param( account, dbip, server_id )
    return 'account='..account..'&ip='..dbip..'&serverid='..server_id
end

function PlatformSqwApplePB:get_login_param_json( )
...
function PlatformSqwApplePB:set_access_token(value)
    self.access_token = value
end

-- 获取登录服务器ip
function PlatformSqwApplePB:getServerIP()
	--UpdateManager.servlist_url = nil
	local platform_id =  Target_Platform
	local _url = CCAppConfig:sharedAppConfig():getStringForKey('server_list')
	print("PlatformSqwApplePB:getServerIP:",_url)
	if _url == '' then
...
-- 获取平台支付地址
function PlatformSqwApplePB:getPlatformPayUrl()
	return ""
end

--登出
function PlatformSqwApplePB:logout(callbackfunc)
	if callbackfunc then
		callbackfunc()
	end
	-- local json_table_temp = {}
	-- json_table_temp[ "message_type" ]	= PlatformSqwApplePB.MESSAGE_TYPE	--消息类型，必传字段
	-- json_table_temp[ "function_type" ] = PlatformSqwApplePB.FUNC_LOGIN_OUT
	-- require 'json/json'
	-- local jcode = json.encode( json_table_temp )
	-- send_message_to_java( jcode )	
...
function PlatformSqwApplePB:lost_connect( callbackfunc )
	if callbackfunc then
		callbackfunc()
	end
	MUtils:toast(LangCommonString[25],2048) -- [25]='返回到登录'
...
--无法登入游戏服务器
function PlatformSqwApplePB:failedTologinGameServer(callbackfunc)
...
-- 上报接口
function PlatformSqwApplePB:notifyZone( ... )
	-- 子类重写
end

--最初的启动
function PlatformSqwApplePB:onStartPackage(cbfunc)
	cbfunc()
end

--开始更新
function PlatformSqwApplePB:onStartUpdate(cbfunc)
	cbfunc()
end

--更新完毕等待进入游戏
function PlatformSqwApplePB:onStartGame(cbfunc)
	cbfunc()
end

function PlatformSqwApplePB:doNeedLogin()
end

-- 按下登录按钮的回调
function PlatformSqwApplePB:doNeedLogin_delay()
end

function PlatformSqwApplePB:OnAsyncMessage( id, msg )
...
function PlatformSqwApplePB:get_pay_config_info()
	require "../data/chong_zhi_config"
	return ChongZhiConf.sqw_apple_pb;
end


--平台主动行为封装 Begin
--支付
function PlatformSqwApplePB:pay(...)
...
function PlatformSqwApplePB:getDownloadFrom()
	return 'noplatform'
end

function PlatformSqwApplePB:showPlatformUI(bFlag)
end
...
function PlatformSqwApplePB:payUICallback( info )
	...
	ZXLog ("-- PlatformSqwApplePB 平台 ---", _orderId, _item_id,_item_index,_userId,_serverId)
...
-- userId:用户id
	...
	local json = string.format("{\"funcID\":\"sdk_sqwpb_purchase\",\"productID\":\"%s\",\"productName\":\"%s\",\"price\":%d,\"serverid\":%d,\"outOrderID\":\"%s\"}",productID, productName, _item_id/10, serId, _orderId)

	IOSDispatcher:async_send_to_oc(json,purchase_callback)
end


function PlatformSqwApplePB:getLoginRet()
	return self.uid, self.token
end


function PlatformSqwApplePB:getPayInfo()
	require "../data/chong_zhi_config"
	return ChongZhiConf.sqw_apple_pb;
end
--------------------------------------
...
function PlatformSqwApplePB:share(info)

end

function PlatformSqwApplePB:setLoginRet(uid,psw)
...
-- 打开用户中心
function PlatformSqwApplePB:open_user_center()
	
end

function PlatformSqwApplePB:onLoginResult( err, data )
	-- body
end
