...
--获取平台登录url
function PlatformMI:get_login_url()
...
--获取服务器列表url
function PlatformMI:get_servlist_url()
	return GameUrl_global:getServerIP() or ""
end

function PlatformMI:show_logout(callbackfunc)
	callbackfunc()
end


--从选服界面登出
function PlatformMI:logoutFromSelectServer(callbackfunc, reason)
...
-- function PlatformMI:get_servlist_param()
...
-- 获取登录服务器ip
function PlatformMI:getServerIP()
	--UpdateManager.servlist_url = nil
	local platform_id =  Target_Platform
	local _url = CCAppConfig:sharedAppConfig():getStringForKey('server_list')
	print("PlatformMI:getServerIP:",_url)
	if _url == '' then
...
-- 获取平台支付地址
function PlatformMI:getPlatformPayUrl()
	return ""
end

--登出
function PlatformMI:logout(callbackfunc)
	...
	local json_table_temp = {}
	json_table_temp[ "message_type" ]	= PlatformMI.MESSAGE_TYPE	--消息类型，必传字段
	json_table_temp[ "function_type" ] = PlatformMI.FUNC_LOGIN_OUT
	require 'json/json'
	local jcode = json.encode( json_table_temp )
	send_message_to_java( jcode )	
...
function PlatformMI:lost_connect( callbackfunc )
	if callbackfunc then
		callbackfunc()
	end
	MUtils:toast(LangCommonString[25],2048) -- [25]='返回到登录'
...
--无法登入游戏服务器
function PlatformMI:failedTologinGameServer(callbackfunc)
...
-- 上报接口
function PlatformMI:notifyZone( ... )
	-- 子类重写
end

--最初的启动
function PlatformMI:onStartPackage(cbfunc)
	cbfunc()
end

--开始更新
function PlatformMI:onStartUpdate(cbfunc)
	cbfunc()
end

--更新完毕等待进入游戏
function PlatformMI:onStartGame(cbfunc)
	cbfunc()
end

function PlatformMI:doNeedLogin()
end

-- 按下登录按钮的回调
function PlatformMI:doNeedLogin_delay()
end
...
function PlatformMI:getDownloadFrom()
	return 'noplatform'
end

function PlatformMI:showPlatformUI(bFlag)
end
...
function PlatformMI:getPayInfo()
	require "../data/chong_zhi_config"
	return ChongZhiConf.xiaomi
end
--------------------------------------
--unused function
--HJH 2014-11-27
--写这个架构的人没想清楚，怎么会有下面这些函数
function PlatformMI:share(info)

end

function PlatformMI:setLoginRet(uid,psw)
...
-- 打开用户中心
function PlatformMI:open_user_center()
	
end

function PlatformMI:onLoginResult( err, data )
	-- body
end