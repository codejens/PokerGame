...
PlatformZS.MESSAGE_TYPE	= "platform"

PlatformZS.FUNC_CHANGE_ACCOUNT = "change_account"

PlatformZS.FUNC_LOGIN		= "login"		--登录

PlatformZS.FUNC_PAY		= "pay"			--支付

PlatformZS.FUNC_TAB		= "tab"			--切换账号

PlatformZS.FUNC_LOGIN_OUT	= "login_out"	--登出

PlatformZS.FUNC_INIT_ROLE	= "init_role"	--传送

PlatformZS.CODE_SUCCESS = 0

PlatformZS.STATE_LOGIN_SUCCESS = 0 -- 成功

PlatformZS.STATE_LOGIN_FAIL = 1	-- 失败
...
function PlatformZS:get_server_filter_list()
...
function PlatformZS:onEnterGameScene(isEnter)
...
function PlatformZS:onEnterLoginState(state)

	RoleModel:show_login_win( GameStateManager:get_game_root() )

	-- if self.firstLogin == true then

	MUtils:toast("掌尚平台登陆请求授权",2048,3) -- [69]="请求登录授权"
...
function PlatformZS:doLogin()

	--跳转到无平台登陆页，有平台应该调用SDK

	-- RoleModel:change_login_page( "login" )
	MUtils:toast("PlatformZS:doLogin", 2048, 3)

	-- RoleModel:change_login_page( "login" )

	local json_table_temp = {}

	json_table_temp[ "message_type" ]	= PlatformZS.MESSAGE_TYPE	--消息类型，必传字段

	json_table_temp[ "function_type" ] = PlatformZS.FUNC_LOGIN
...
function PlatformZS:get_login_url()
...
function PlatformZS:get_servlist_url()
...
function PlatformZS:show_logout(callbackfunc)
...
function PlatformZS:logoutFromSelectServer(callbackfunc, reason)
...
-- function PlatformZS:get_servlist_param()
...
function PlatformZS:get_login_param( account, dbip, server_id )
...
function PlatformZS:get_login_param_json( )
...
function PlatformZS:getServerIP()
	...
	print("PlatformZS:getServerIP:",_url)
...
function PlatformZS:getPlatformPayUrl()
...
function PlatformZS:logout(callbackfunc)
	...
	json_table_temp[ "function_type" ] = PlatformZS.FUNC_LOGIN_OUT
...
function PlatformZS:lost_connect( callbackfunc )
...
function PlatformZS:failedTologinGameServer(callbackfunc)
...
function PlatformZS:notifyZone( ... )
...
function PlatformZS:onStartPackage(cbfunc)
...
function PlatformZS:onStartUpdate(cbfunc)
...
function PlatformZS:onStartGame(cbfunc)
...
function PlatformZS:doNeedLogin()
...
function PlatformZS:doNeedLogin_delay()

end



function PlatformZS:OnAsyncMessage( id, msg )
...
function PlatformZS:get_cache_url()
...
function PlatformZS:get_package_url()
...
function PlatformZS:getDownloadFrom()
...
function PlatformZS:showPlatformUI(bFlag)

end


--支付
function PlatformZS:payUICallback( info )
	...
	json_table_temp[ "function_type" ] = PlatformZS.FUNC_PAY
	...
	json_table_temp[ "goodsDesc" ] =  _item_id.."元宝"

	json_table_temp[ "goods" ] = _item_id.."元宝"
...
function PlatformZS:switch_account()
	local json_table_temp = {}
	json_table_temp[ "message_type" ]	= PlatformZS.MESSAGE_TYPE	--消息类型，必传字段
	json_table_temp[ "function_type" ] = PlatformZS.FUNC_LOGIN
	require 'json/json'
...
function PlatformZS:getLoginRet()
...
function PlatformZS:getPayInfo()

	require "../data/chong_zhi_config"

	return ChongZhiConf.zhangshang
...
function PlatformZS:share(info)
...
function PlatformZS:setLoginRet(uid,psw)
...
function PlatformZS:open_user_center()
...
function PlatformZS:onLoginResult( err, data )
