PlatformEwan= {}
PlatformEwan.MESSAGE_TYPE	= "platform"
PlatformEwan.FUNC_CHANGE_ACCOUNT = "change_account"
PlatformEwan.FUNC_LOGIN		= "login"		--登录
PlatformEwan.FUNC_PAY		= "pay"			--支付
PlatformEwan.FUNC_TAB		= "tab"			--切换账号
PlatformEwan.FUNC_LOGIN_OUT	= "login_out"	--登出
PlatformEwan.FUNC_QUIT		= "quit"		--退出
PlatformEwan.FUNC_INIT_ROLE	= "init_role"	--传送
PlatformEwan.FUNC_USER_CENTER = "user_center"
PlatformEwan.FUNC_SWITCH_ACCOUNT = "swithc_account"
PlatformEwan.FUNC_ENTRY_PLATFORM = "enter_platform"

PlatformEwan.CODE_SUCCESS = 0
PlatformEwan.STATE_LOGIN_SUCCESS = 0 -- 成功
PlatformEwan.STATE_LOGIN_FAIL = 1	-- 失败

-- 不准许有全局参数，因为需要被reload
--
--
function PlatformEwan:init()
	self.firstLogin = true
	self.loginRet = nil
	self.tokens = nil
	self.logoutable = true
	self.show_btn = 1
	self.slashscreen = { 'platform_800_480.pd', 'hoolai_800_480.pd' }
	self.download_url =  ''
	self.home_url     = CommonConfig.home
	--http是否已经返回！
	self.payurl_waiting     = false
	self.unlockcallback = callback:new()
	self.access_token = ""
	self.openId = ""
	self.user_name = ""
	self.AppId = ''

end

function PlatformEwan:get_server_filter_list()
	return nil--CCAppConfig:sharedAppConfig():getStringForKey('develop_serverFilter')
end

-- @param:是否进入或者离开游戏场景,true：进入,false:离开
function PlatformEwan:onEnterGameScene(isEnter)
	if isEnter == true then

	else

	end
end

-- /*
--  * 参数一：1为登录动作，2为创建角色（必传）
--  * 参数二：区服标识id（必传）
--  * 参数三：区服名称（必传）
--  * 参数四：角色id（必传）
--  * 参数五：角色名称（必传）
--  * 参数六：角色等级（必传）
--  * 参数七：扩展字段（可选）
--  */
-- String roleName = resultJson.getString("roleName");
-- String serverId = resultJson.getString("serverId");
-- String serverName = resultJson.getString("serverName");
-- String roleId = resultJson.getString("roleId");
-- String roleLevel = resultJson.getString("roleLevel");
function PlatformEwan:init_role_info()
	local login_info = RoleModel:get_login_info()
	local serverId = login_info.server_id
	local serverName = RoleModel:get_server_name_had_login(  )
	local player = EntityManager:get_player_avatar()
	local roleId = player.id
	local roleName = player.name
	local roleLevel = player.level
	local json_table_temp = {}
	json_table_temp[ "message_type" ]	= PlatformEwan.MESSAGE_TYPE	--消息类型，必传字段
	json_table_temp[ "function_type" ] = PlatformEwan.FUNC_INIT_ROLE
	json_table_temp[ "serverId" ] = tostring(serverId)
	json_table_temp[ "serverName" ] = serverName
	json_table_temp[ "roleId" ] = tostring(roleId)
	json_table_temp[ "roleName" ] = roleName
	json_table_temp[ "roleLevel" ] = tostring(roleLevel)
	require 'json/json'
	local jcode = json.encode( json_table_temp )
	send_message_to_java( jcode )
end

-- 入口,当Login界面Show
-- 显示登陆界面
function PlatformEwan:onEnterLoginState(state)
	RoleModel:show_login_win( GameStateManager:get_game_root() )
	if self.firstLogin == true then
		-- MUtils:toast("Ewan 登陆请求授权",2048,3) -- [69]="请求登录授权"
		local c = callback:new()
			c:start(0.1,function()
				PlatformEwan:doLogin()
			end)
		self.firstLogin = false
	end
end

-- 按下登录按钮的回调，应显示平台登陆页
function PlatformEwan:doLogin()
	--跳转到无平台登陆页，有平台应该调用SDK
	-- RoleModel:change_login_page( "login" )
	-- MUtils:toast("PlatformEwan:doLogin", 2048, 3)

	-- RoleModel:change_login_page( "login" )
	local json_table_temp = {}
	json_table_temp[ "message_type" ]	= PlatformEwan.MESSAGE_TYPE	--消息类型，必传字段
	json_table_temp[ "function_type" ] = PlatformEwan.FUNC_LOGIN
	require 'json/json'
	local jcode = json.encode( json_table_temp )
	send_message_to_java( jcode )
end

--获取平台登录url
function PlatformEwan:get_login_url()
	--UpdateManager.login_url = nil
	local platform_id =  Target_Platform
	local _url = CCAppConfig:sharedAppConfig():getStringForKey('server_list')
	if _url == '' then
		_url = UpdateManager.server_url .. 'noplatform/'
	end
	return _url
end

--获取服务器列表url
function PlatformEwan:get_servlist_url()
	return GameUrl_global:getServerIP() or ""
end

function PlatformEwan:show_logout(callbackfunc)
	callbackfunc()
end


--从选服界面登出
function PlatformEwan:logoutFromSelectServer(callbackfunc, reason)
	print('PlatformEwan:logoutFromSelectServer',err, data)
	if callbackfunc then
		callbackfunc()
	end

	-- if reason then
	-- 	MUtils:toast(reason,2048)
	-- end
	--跳转到无平台登陆页，有平台应该调用SDK
	--并且隐藏窗口
	RoleModel:change_login_page( "loginSDK" )
end

-- -- 获取该平台，发送请求服务器列表接口的http参数
-- function PlatformEwan:get_servlist_param()
-- 	local account, pwd = self:getLoginRet()
-- 	if account then
-- 		return string.format('account=%s&pw=%s',account,md5(pwd));
-- 	else
-- 		return ''
-- 	end
-- end

-- login 的参数
function PlatformEwan:get_login_param( account, dbip, server_id )
    return 'account='..account..'&ip='..dbip..'&serverid='..server_id .. "&channel=" .. CCAppConfig:sharedAppConfig():getStringForKey('lh_channel')
end

function PlatformEwan:get_login_param_json( )
	--env=release&openid=?&token=?
    -- return 'openid=' ..user_name.."&pw="..md5(password)
    return "env=release" .. "&openid=" .. self.openId  .. "&token=" .. self.access_token .. "&channel=" .. CCAppConfig:sharedAppConfig():getStringForKey('lh_channel')
    --return "token=" .. ZXLuaUtils:URLEncode(self.access_token) .. "&channel=" .. CCAppConfig:sharedAppConfig():getStringForKey('lh_channel')
end


-- 获取登录服务器ip
function PlatformEwan:getServerIP()
	--UpdateManager.servlist_url = nil
	local platform_id =  Target_Platform
	local _url = CCAppConfig:sharedAppConfig():getStringForKey('server_list')
	print("PlatformEwan:getServerIP:",_url)
	if _url == '' then
		_url = UpdateManager.servlist_url
		print("UpdateManager.servlist_url",servlist_url)
	end
	return _url
end

-- 获取平台支付地址
function PlatformEwan:getPlatformPayUrl()
	return ""
end

--登出
function PlatformEwan:logout(callbackfunc)
	-- print("run loginout11111")
	if callbackfunc then
		callbackfunc()
	end
	-- print("run loginout22222")
	local json_table_temp = {}
	json_table_temp[ "message_type" ]	= PlatformEwan.MESSAGE_TYPE	--消息类型，必传字段
	json_table_temp[ "function_type" ] = PlatformEwan.FUNC_LOGIN_OUT
	require 'json/json'
	local jcode = json.encode( json_table_temp )
	-- print("run loginout33333")
	send_message_to_java( jcode )	
	-- print("run loginout44444")
	MUtils:toast(LangCommonString[25],2048) -- [25]='返回到登录'
	-- print("run loginout55555")
end

function PlatformEwan:lost_connect( callbackfunc )
	if callbackfunc then
		callbackfunc()
	end
	MUtils:toast(LangCommonString[25],2048) -- [25]='返回到登录'
end


--无法登入游戏服务器
function PlatformEwan:failedTologinGameServer(callbackfunc)

	if callbackfunc then
		callbackfunc()
	end

	RoleModel:destroy_login_without_update_win()
end


-- 上报接口
function PlatformEwan:notifyZone( ... )
	-- 子类重写
end

--最初的启动
function PlatformEwan:onStartPackage(cbfunc)
	cbfunc()
end

--开始更新
function PlatformEwan:onStartUpdate(cbfunc)
	cbfunc()
end

--更新完毕等待进入游戏
function PlatformEwan:onStartGame(cbfunc)
	cbfunc()
end

function PlatformEwan:doNeedLogin()
end

-- 按下登录按钮的回调
function PlatformEwan:doNeedLogin_delay()
end

function PlatformEwan:OnAsyncMessage( id, msg )
	ZXLog('PlatformNoPlatform:OnAsyncMessage', id, msg)
	require 'json/json'
	local jtable = {}
	local s,e = pcall(function() jtable = json.decode(msg) end)
	-- MUtils:toast("成功进入！", 2048, 2.5)
	print("s,e",s,e)
	if s then
		local message_type = jtable[ "message_type" ] or ""
		print("message_type",message_type)
		if message_type == self.MESSAGE_TYPE then
			local func_type = jtable[ "funcType" ] or ""
			local error_code = jtable[ "error_code" ] or self.CODE_FAIL
			print("func_type",func_type)
			print('error_code',error_code)
			if func_type == self.FUNC_LOGIN then -- 登录、切换成功的处理
				if error_code == self.CODE_SUCCESS then
					MUtils:toast("登陆成功", 2048, 2.5)
					self.access_token = jtable["token"]
					self.openId = jtable["openId"]
					self.user_name = jtable["userName"]
					self.show_btn = jtable["showSwitch"]
					RoleModel:request_server_list_platform()
				else
					MUtils:toast("登陆失败", 2048, 2.5)				
				end
				return true
			elseif func_type == self.FUNC_CHANGE_ACCOUNT then
					MiscCC:send_quit_server()
					GameStateManager:set_state("login")
					-- RoleModel:send_name_and_pw(self.uid, self.access_token, nil, true)
					-- RoleModel:change_login_page("new_select_server_page")
			end
			return true
		end
	end
	MUtils:toast("登陆失败", 2048, 2.5)
	return false;
end

function PlatformEwan:get_cache_url()
	return UpdateManager.cache_url .. "/download" .. CCAppConfig:sharedAppConfig():getStringForKey('target_platform_folder')
end

function PlatformEwan:get_package_url()
	return UpdateManager.package_url .. "/update" .. CCAppConfig:sharedAppConfig():getStringForKey('target_platform_folder')
end

--平台主动行为封装 Begin
--支付
function PlatformEwan:pay(...)
	local payWin = UIManager:show_window("pay_win");
	payWin:setCallback(function(which)
			self:payUICallback({ item_id = which, window = 'pay_win' })
		end)
end

function PlatformEwan:getDownloadFrom()
	return 'noplatform'
end

function PlatformEwan:showPlatformUI(bFlag)
end


--支付
--       //封装支付所需要的参数,以下两种方式即可
--       /*
--        * 参数一：支付价格,float
--        * 参数二：服务器标识
--        * 参数三：游戏货币名称（固定金额时才使用到，可变金额在后台配置）
--        * 参数四：充值描述
--        * 参数五：可换取的货物数量（固定金额时才使用到，可变金额可传0值）
--        * 参数六：自定义字段：多为CP订单
--        */
-- String price = resultJson.getString("price");
-- String serverId = resultJson.getString("serverId");
-- String moneyName = resultJson.getString("moneyName");
-- String exchangeNum = resultJson.getString("exchangeNum");
-- String orderId = resultJson.getString("orderId");
function PlatformEwan:payUICallback( info )

	require 'model/iOSChongZhiModel'
	--iOSChongZhiModel:purchase_product( info.item_id )
	local _money_rate = 0.1
    -- local user_account = RoleModel:get_login_info().user_name

    local _item_id  = info.item_id
    local player = EntityManager:get_player_avatar()
	local _userId   = player.id
	local _userName = RoleModel:get_login_info().user_name
	local _serverId = RoleModel:get_login_info().server_id

	local _orderId = "LionHeart_Ewan".._userId.."_".._serverId.."_"..os.time();

	MUtils:toast("充值中", 2048, 3)

	local json_table_temp = {}
	json_table_temp[ "message_type" ]	= PlatformEwan.MESSAGE_TYPE	--消息类型，必传字段
	json_table_temp[ "function_type" ] = PlatformEwan.FUNC_PAY
	json_table_temp[ "price" ] = _item_id * _money_rate
	json_table_temp[ "serverId" ] = _serverId	
	json_table_temp[ "moneyName" ] = "元宝"
	json_table_temp[ "moneyDesc" ] = string.format("%d元%d元宝", _item_id * _money_rate, _item_id) 
	json_table_temp[ "exchangeNum" ] = _item_id
	json_table_temp[ "orderId" ] = _orderId
	
	require 'json/json'
	local jcode = json.encode( json_table_temp )
	send_message_to_java( jcode )
end


function PlatformEwan:getLoginRet()
	return self.uid, self.token
end


function PlatformEwan:getPayInfo()
	local temp_info = ChongZhiConfig:get_chong_zhi_info()
	return temp_info.ewan
end
--------------------------------------
--unused function
--HJH 2014-11-27
--写这个架构的人没想清楚，怎么会有下面这些函数
function PlatformEwan:share(info)

end

function PlatformEwan:setLoginRet(uid,psw)
	
end

-- 获取该平台，发送请求服务器列表接口的http参数
function PlatformEwan:get_servlist_param(  )
	return ""
end

-- 打开用户中心
function PlatformEwan:open_user_center()
	local json_table_temp = {}
	json_table_temp[ "message_type" ]	= PlatformEwan.MESSAGE_TYPE	--消息类型，必传字段
	json_table_temp[ "function_type" ] = PlatformEwan.FUNC_USER_CENTER
	require 'json/json'
	local jcode = json.encode( json_table_temp )
	send_message_to_java( jcode )	
	print("run PlatformEwan open_user_center")
end

function PlatformEwan:onLoginResult( err, data )
	-- body
end


function PlatformEwan:switch_account()
	local json_table_temp = {}
	json_table_temp[ "message_type" ]	= PlatformEwan.MESSAGE_TYPE	--消息类型，必传字段
	json_table_temp[ "function_type" ] = PlatformEwan.FUNC_SWITCH_ACCOUNT
	require 'json/json'
	local jcode = json.encode( json_table_temp )
	send_message_to_java( jcode )
end

function PlatformEwan:is_enter_platform()
	local json_table_temp = {}
	json_table_temp[ "message_type" ]	= PlatformEwan.MESSAGE_TYPE	--消息类型，必传字段
	json_table_temp[ "function_type" ] = PlatformEwan.FUNC_ENTRY_PLATFORM
	require 'json/json'
	local jcode = json.encode( json_table_temp )
	send_message_to_java( jcode )
end

function PlatformEwan:is_show_btn( )
	return self.show_btn
end
--退出游戏
function PlatformEwan:exit()
	local json_table_temp = {}
	json_table_temp[ "message_type" ]	= self.MESSAGE_TYPE					--消息类型，必传字段
	json_table_temp[ "function_type" ]		= self.FUNC_QUIT					--分发类型，必传字段
	require 'json/json'
	local jcode = json.encode( json_table_temp )
	send_message_to_java(jcode)
end