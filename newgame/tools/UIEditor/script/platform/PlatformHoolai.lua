...
PlatformHoolai = {}
--消息类型
PlatformHoolai.MESSAGE_TYPE			= "platform"
--分发类型
PlatformHoolai.FUNC_LOGIN				= "login"		--登录
PlatformHoolai.FUNC_PAY				= "pay"			--支付
PlatformHoolai.FUNC_HL_PAY			= "hl_pay"			--hoolai支付
PlatformHoolai.FUNC_QUIT        		= "quit"        --退出平台
PlatformHoolai.FUNC_PAY_BACK     		= "pay_back"    --充值返回
PlatformHoolai.FUNC_PAY_NEED_LOGIN    = "pay_need_login"    --充值时 登录状态失效需要玩家重新登录
PlatformHoolai.pay_url				= "http://loginlongjian.hoolaigames.com/msdk/yyb_zy" --支付地址
PlatformHoolai.pay_url2 				= "http://loginlongjian.hoolaigames.com/msdk/yyb_tx" --支付地址
PlatformHoolai.FUNC_BIND_PAY_SERVICE 	= "bind_pay_service"
PlatformHoolai.FUNC_REMOVE_PAY_ERVICE = "remove_pay_ervice"
PlatformHoolai.FUNC_PAY_NEED_LOGIN 	= "pay_need_login"
PlatformHoolai.FUNC_PAY_RESULT		= "pay_result"
PlatformHoolai.FUNC_NOTIC_INFO		= "notic_info"
PlatformHoolai._platform_qq			= 2
PlatformHoolai._platform_wx			= 1

--充值返回结果信息
PlatformHoolai.pay_resultCode = 
{	
	--说明游戏初始化绑定service不成功
...
-- local _wxRefreshToken = ""
	--初始化
function PlatformHoolai:init(callbackfunc)
...
--进入登录状态
function PlatformHoolai:onEnterLoginState(isFirlstLoad)
	...
	-- 	c:start(0.1,function() PlatformHoolai:doLogin() end)
	-- 	self.firstLogin = false
	-- end
end

function PlatformHoolai:set_login_type(arg)
	self.login_type = arg
end

--按下登录按钮的回调
function PlatformHoolai:doLogin()
	-- MUtils:toast("PlatformYYB:doLogin", 2048, 3)
	local json_table_temp = {}
	json_table_temp[ "message_type" ]	= PlatformHoolai.MESSAGE_TYPE	--消息类型，必传字段
	json_table_temp[ "function_type" ]		= PlatformHoolai.FUNC_LOGIN		--分发类型，必传字段
...
--获取平台登录url
function PlatformHoolai:get_login_url()
...
--获取服务器列表url
function PlatformHoolai:get_servlist_url()
	return GameUrl_global:getServerIP() or ""
end

function PlatformHoolai:show_logout(callbackfunc)
	callbackfunc()
end

--从选服界面登出
function PlatformHoolai:logoutFromSelectServer( callbackfunc, reason )
	...
	PlatformHoolai:showPlatformUI(true)
end

-- login 的参数
function PlatformHoolai:get_login_param( account, dbip, server_id )
    return 'account='..account..'&ip='..dbip..'&serverid='..server_id .. "&channel=" .. CCAppConfig:sharedAppConfig():getStringForKey('lh_channel')
end

function PlatformHoolai:get_login_param_json( )
	print("PlatformHoolai:get_login_param_json self._platform",self._platform)
...
-- 获取登录服务器ip
function PlatformHoolai:getServerIP()
	--UpdateManager.servlist_url = nil
	local platform_id =  Target_Platform
	local _url = CCAppConfig:sharedAppConfig():getStringForKey('server_list')
	print("PlatformHoolai:getServerIP:",_url)
	if _url == '' then
...
-- 获取平台支付地址
function PlatformHoolai:getPlatformPayUrl()
	return ""
end

--登出
function PlatformHoolai:logout( callbackfunc )
	if callbackfunc then
		callbackfunc()
	end
...
function PlatformHoolai:lost_connect( callbackfunc )
	if callbackfunc then
		callbackfunc()
	end
	MUtils:toast(LangCommonString[25], 2048 )
...
--无法登入游戏服务器
function PlatformHoolai:failedTologinGameServer(callbackfunc)
...
-- 上报接口
function PlatformHoolai:notifyZone( ... )
	-- 子类重写
end

--最初的启动
function PlatformHoolai:onStartPackage(cbfunc)
	cbfunc()
end


--开始更新
function PlatformHoolai:onStartUpdate(cbfunc)
	cbfunc()
end

--更新完毕等待进入游戏
function PlatformHoolai:onStartGame(cbfunc)
	cbfunc()
end


function PlatformHoolai:init_role_info()
	self:requst_server_about_pay()
end
--初始化登录的服务器信息
function PlatformHoolai:set_server_info( server_info )
	self.loginRet = server_info 
end
--JSON消息接收与处理
function PlatformHoolai:OnAsyncMessage( id, msg )
	print("PlatformHoolai:OnAsyncMessage",id, msg)
	--过滤掉不是平台返回的信息
	...
	if s then
		...
		if message_type == PlatformHoolai.MESSAGE_TYPE then
			local funcType = jtable[ "funcType" ] or ""
			if funcType == PlatformHoolai.FUNC_LOGIN then
				--登录
				...
				if self._platform == PlatformHoolai._platform_qq then
					self._platform = "qq"
				elseif self._platform == PlatformHoolai._platform_wx then
					self._platform = "wx"
				end
	        	RoleModel:request_server_list_platform()
	        elseif funcType == PlatformHoolai.FUNC_PAY_RESULT then 
	        	print("PlatformHoolai.FUNC_PAY_RESULT")
	        	--平台充值回调
			    ...
			    local pay_result_type = PlatformHoolai.pay_resultCode
			    ...
			    elseif resultCode ==pay_result_type.PAYRESULT_SUCC  then
			    	...
			    	family = PlatformHoolai.pay_family[str]
			    	-- ZXLog("渠道-------------------：",family)
			    	self:requst_server_about_pay()
			...
			elseif funcType ==  PlatformHoolai.FUNC_PAY_NEED_LOGIN then 
				--登录失效。需要重新登录
				-- ZXLog("登录失效。需要重新登录")
				GameStateManager:back_to_login()
			elseif funcType == PlatformHoolai.FUNC_NOTIC_INFO then
				local notic_type = jtable["msg"]
				notic_type = tonumber(notic_type)
				if PlatformHoolai.login_error[notic_type] then
					MUtils:toast( PlatformHoolai.login_error[notic_type], 2048 )
				else
...
--与服务器通信，通知支付情况
function PlatformHoolai:requst_server_about_pay()
	print("与服务器通信，通知支付情况")
	 -- 请求回调
	...
	local pay_url = Target_Platform == Platform_Type.Platform_Hoolai and PlatformHoolai.pay_url or PlatformHoolai.pay_url2
	local url = pay_url.."/change"
	local param = self:get_pay_result_param()
	print("PlatformHoolai:requst_server_about_pay:", url, param)
	HttpRequest.sendPay( url, param, httpPayResponse )
	current_request = current_request+1
end

--平台返回支付结果后 跟服务器通信 参数
function PlatformHoolai:get_pay_result_param()
	--服务器需要参数
...
function PlatformHoolai:get_servlist_param(  )
...
function PlatformHoolai:get_pay_log(  )
...
--支付
function PlatformHoolai:payUICallback( info )
	if UpdateManager.msdk_pay_type == "hoolai" then
 ...
 --        end 
	-- end
	-- --通知后台玩家点击了充值按钮
	-- local url = PlatformHoolai.pay_url.."/pay_log"
	...
	local json_table_temp = {}
	json_table_temp[ "message_type" ]	= PlatformHoolai.MESSAGE_TYPE	--消息类型，必传字段
	json_table_temp[ "function_type" ] = PlatformHoolai.FUNC_PAY              --分发类型，必传字段
...
--支付2 胡莱支付
function PlatformHoolai:payUICallback2( info )
	local item_id  = info.item_id
	...
	local json_table_temp = {}
	json_table_temp[ "message_type" ]	= PlatformHoolai.MESSAGE_TYPE	--消息类型，必传字段
	json_table_temp[ "function_type" ] = PlatformHoolai.FUNC_HL_PAY              --分发类型，必传字段
...
function PlatformHoolai:showPlatformUI( bFlag )
...
function PlatformHoolai:getLoginRet(  )
	return nil
end
function PlatformHoolai:getUserRet()
