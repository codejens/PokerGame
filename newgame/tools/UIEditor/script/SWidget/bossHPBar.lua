...
function bossHPBar:set_hp(hp, maxHp)
	self.maxHp = maxHp
	self.curHp = hp
...
function bossHPBar:update_blood(changeHp, hp, maxHp)
	...
	local function cb()
		...
		if self.bloodBgIndex == self.curBloodIndex then
			...
			-- printc("做了直接赋值给self,.whiteLeftBlood", self.lastLeftBlood, self.whiteLeftBlood, self.singleLeftBlood, self.minusSpeed, 14)
...
function bossHPBar:get_handle_blood_index(hp, maxHp)
	-- 获得当前操作的血条的索引
	local index = math.floor((maxHp - hp) / self.singleBlood) + 1
	self.singleLeftBlood = self.singleBlood - ((maxHp - hp) - self.singleBlood * (index - 1))
	-- printc(" bossHPBar:get_handle_blood_index=======>>>>", hp, maxHp, self.singleBlood, self.curBloodIndex, self.singleLeftBlood, index, 13)
