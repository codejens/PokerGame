...
--加载情人节活动的配置对象
require "config/activity/WomensConfig"
...
--创建情人节活动数据逻辑
WomensDayModel = BaseCommonModel(nil, WomensConfig)
...
--作者：陈亮
--时间：2014.09.19
function WomensDayModel:showWomensDayWin(isInit)
	--获取情人节活动窗口，因为是显示窗口才调这个函数，所以不用判断
	local t_womens_day_win = UIManager:find_visible_window("womens_day_win")
...
--作者：郭志楠
--时间：2015.2.6
function WomensDayModel:showVLuoPanPage(isInit)
	--获取情人节活动窗口，因为是显示窗口才调这个函数，所以不用判断
	local t_win = UIManager:find_visible_window("womens_day_win")
...
--作者：肖进超
--时间：2014.12.17
function WomensDayModel:showCExchangePage(isInit)
	local t_win = UIManager:find_visible_window("womens_day_win")
...
--作者：肖进超
--时间：2014.12.17
function WomensDayModel:showCTaobaoPage(isInit)
    local t_win = UIManager:find_visible_window("womens_day_win")
...
--作者：陈亮
--时间：2014.10.27
function WomensDayModel:showSendFollowQueuePage(isInit)
	--获取情人节活动窗口，因为是显示窗口才调这个函数，所以不用判断
	local t_womens_day_win = UIManager:find_visible_window("womens_day_win")
...
--作者：陈亮
--时间：2014.10.27
function WomensDayModel:showReceiveFollowQueuePage(isInit)
	--获取情人节活动窗口，因为是显示窗口才调这个函数，所以不用判断
	local t_womens_day_win = UIManager:find_visible_window("womens_day_win")
...
--作者：肖进超
--时间：2014.10.29
function WomensDayModel:showLDTreasurePage(isInit)
	--获取情人节活动窗口，因为是显示窗口才调这个函数，所以不用判断
	local t_win = UIManager:find_visible_window("womens_day_win")
...
--作者：肖进超
--时间：2014.10.29
function WomensDayModel:showLDRepeatConsumePage(isInit)
...
--作者：肖进超
--时间：2014.10.29
function WomensDayModel:showLDGroupBuyPage(isInit)
	--获取情人节活动窗口，因为是显示窗口才调这个函数，所以不用判断
	local t_win = UIManager:find_visible_window("womens_day_win")
...
--作者：肖进超
--时间：2014.10.14
function WomensDayModel:showLDAddupLoginPage(isInit)
	--获取情人节活动窗口，因为是显示窗口才调这个函数，所以不用判断
	local t_win = UIManager:find_visible_window("womens_day_win")
...
--作者：肖进超
--时间：2014.10.14
function WomensDayModel:showLDBossHitPage(isInit)
	--获取情人节活动窗口，因为是显示窗口才调这个函数，所以不用判断
	local t_win = UIManager:find_visible_window("womens_day_win")
...
--作者：谢汉德
--时间：2014.10.14
function WomensDayModel:showDailyQuotaGuyPage(isInit)
...
--作者：肖进超
--时间：2014.10.14
function WomensDayModel:showLDBoxDownPage(isInit)
	--获取情人节活动窗口，因为是显示窗口才调这个函数，所以不用判断
	local t_win = UIManager:find_visible_window("womens_day_win")
...
function WomensDayModel:refreshPageInfo(page_type)
	local t_win = UIManager:find_visible_window("womens_day_win")
	--如果没有显示，不进行操作
	if (not t_win) then
		return
...
--作者：肖进超
--时间：2014.12.17
function WomensDayModel:showCEveryConsumePage(isInit)
	local t_win = UIManager:find_visible_window("womens_day_win")
...
--作者：肖进超
--时间：2014.10.30
function WomensDayModel:flushPageInfo(activityInfo)
	--获取情人节活动窗口
	local t_win = UIManager:find_visible_window("womens_day_win")
	--如果没有显示，不进行操作
	if (not t_win) then
...
--作者：陈亮
--时间：2014.08.30
function WomensDayModel:finish()
	UIManager:destroy_window("womens_day_win")
end