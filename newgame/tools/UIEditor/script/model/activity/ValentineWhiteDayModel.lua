--ValentineWhiteDayModel.lua
--内容：情人节活动的单体数据逻辑类
--作者：陈亮
--时间：2014.10.27

--加载情人节活动的配置对象
require "config/activity/ValentineWhiteConfig"
...
local _SEND_FOLLOW_QUEUE_PARAM_INDEX = 2
local _RECEIVE_FOLLOW_QUEUE_PARAM_INDEX = 3
local _APPUP_LOGIN_PARAM_INDEX 	    = 4
local _GROUP_BUY_PARAM_INDEX 	    = 5

--创建情人节活动数据逻辑
ValentineWhiteDayModel = BaseCommonModel(nil,ValentineWhiteConfig)
...
--作者：陈亮
--时间：2014.09.19
function ValentineWhiteDayModel:showValentineWhiteDayWin(isInit)
	--获取情人节活动窗口，因为是显示窗口才调这个函数，所以不用判断
	local t_valentineDayWin = UIManager:find_visible_window("valentineWhiteDayWin")
...
--作者：郭志楠
--时间：2015.2.6
function ValentineWhiteDayModel:showVLuoPanPage(isInit)
	--获取情人节活动窗口，因为是显示窗口才调这个函数，所以不用判断
	local t_win = UIManager:find_visible_window("valentineWhiteDayWin")
...
--作者：肖进超
--时间：2014.12.17
function ValentineWhiteDayModel:showCExchangePage(isInit)
	local t_win = UIManager:find_visible_window("valentineWhiteDayWin")
...
--作者：肖进超
--时间：2014.12.17
function ValentineWhiteDayModel:showCTaobaoPage(isInit)
    local t_win = UIManager:find_visible_window("valentineWhiteDayWin")
...
--作者：陈亮
--时间：2014.10.27
function ValentineWhiteDayModel:showSendFollowQueuePage(isInit)
	--获取情人节活动窗口，因为是显示窗口才调这个函数，所以不用判断
	local t_valentineDayWin = UIManager:find_visible_window("valentineWhiteDayWin")
...
--作者：陈亮
--时间：2014.10.27
function ValentineWhiteDayModel:showReceiveFollowQueuePage(isInit)
	--获取情人节活动窗口，因为是显示窗口才调这个函数，所以不用判断
	local t_valentineDayWin = UIManager:find_visible_window("valentineWhiteDayWin")
...
--作者：肖进超
--时间：2014.10.29
function ValentineWhiteDayModel:showLDTreasurePage(isInit)
	--获取情人节活动窗口，因为是显示窗口才调这个函数，所以不用判断
	local t_win = UIManager:find_visible_window("valentineWhiteDayWin")
...
--作者：肖进超
--时间：2014.10.29
function ValentineWhiteDayModel:showLDRepeatConsumePage(isInit)
...
--作者：肖进超
--时间：2014.10.29
function ValentineWhiteDayModel:showLDGroupBuyPage(isInit)
	--获取情人节活动窗口，因为是显示窗口才调这个函数，所以不用判断
	local t_win = UIManager:find_visible_window("valentineWhiteDayWin")
...
--作者：肖进超
--时间：2014.10.14
function ValentineWhiteDayModel:showLDAddupLoginPage(isInit)
	--获取情人节活动窗口，因为是显示窗口才调这个函数，所以不用判断
	local t_win = UIManager:find_visible_window("valentineWhiteDayWin")
...
--作者：肖进超
--时间：2014.10.14
function ValentineWhiteDayModel:showLDBossHitPage(isInit)
	--获取情人节活动窗口，因为是显示窗口才调这个函数，所以不用判断
	local t_win = UIManager:find_visible_window("valentineWhiteDayWin")
...
--作者：谢汉德
--时间：2014.10.14
function ValentineWhiteDayModel:showDailyQuotaGuyPage(isInit)
...
--作者：肖进超
--时间：2014.10.14
function ValentineWhiteDayModel:showLDBoxDownPage(isInit)
	--获取情人节活动窗口，因为是显示窗口才调这个函数，所以不用判断
	local t_win = UIManager:find_visible_window("valentineWhiteDayWin")
...
function ValentineWhiteDayModel:refreshPageInfo(page_type)
	local t_win = UIManager:find_visible_window("valentineWhiteDayWin")
	--如果没有显示，不进行操作
	if (not t_win) then
		return
...
--作者：肖进超
--时间：2014.12.17
function ValentineWhiteDayModel:showCEveryConsumePage(isInit)
	local t_win = UIManager:find_visible_window("valentineWhiteDayWin")
...
--作者：肖进超
--时间：2014.10.30
function ValentineWhiteDayModel:flushPageInfo(activityInfo)
	--获取情人节活动窗口
	local t_win = UIManager:find_visible_window("valentineWhiteDayWin")
	--如果没有显示，不进行操作
	if (not t_win) then
...
--作者：陈亮
--时间：2014.08.30
function ValentineWhiteDayModel:finish()
	UIManager:destroy_window("valentineWhiteDayWin")
end
...
--作者：chj
--时间：2014.08.30
function ValentineWhiteDayModel:openGroupBuyActivityWin()
	SuperGroupBuyModel:set_ACTIVITY_ID( CommonActivityConfig.ValentineWhiteDay )
