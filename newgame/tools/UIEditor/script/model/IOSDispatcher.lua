...
--  发往OC层的json格式必须包含: 1、funcID字段,标识哪个函数消息;
--  如:  json = { "funcID" : "iap_purchase"} 
--	或是带参数的 json = { "funcID" : "iap_purchase","productID":"mieshenProduct1"}


function IOSDispatcher:fini( )
...
function IOSDispatcher:async_send_to_oc( json, luaHandler )
	-- luaHandler 函数 应该至少接受2个参数, func(error,msg)	
...
function IOSDispatcher:set_login_message( user_account, serId  )
	...
	-- 验证地址
	-- local url = pGameUrl_global:getServerIP()
	--去掉头8个字符，去掉后11个字符
	--最后 只剩下域名 appStore.gdds.shediao.com
	-- local domain = 'loginhy.hoolaigames.com'-- string.sub(url,8,-11)

	-- local url = "hy/apple/pay.jsp"

	local url=PlatformInterface:get_iap_callback()
	print("IOSDispatcher:set_login_message:url=",url)
	...
	print("domain,exten,j,url.length",domain,exten,j,url.length)
	--如果是外网测试环境，则切换一下ip地址
	-- if Target_Platform == PlatformInterface.Platform_Home_Out then
	-- 	domain = "42.62.59.61"
	-- end
...
function IOSDispatcher:IAP_purchase_product( productID, callback )
	...
	local json = string.format("{\"funcID\":\"iap_purchase\",\"productID\":\"%s\",\"userId\":\"%s\",\"serverid\":%d}",productID, user_account, serId)
...
function IOSDispatcher:IAP_add_observer( )
	...
	local json = string.format("{\"funcID\":\"iap_addObserver\"}")
...
function IOSDispatcher:IAP_remove_observer(  )
	local json = string.format("{\"funcID\":\"iap_removeObserver\"}")
	IOSDispatcher:sync_send_to_oc( json )
end

-- 确认充值队列里是有未验证的订单，重新验证
function IOSDispatcher:IAP_reverfy_payment_queue(  )
	...
	local json = string.format("{\"funcID\":\"iap_reverfy\"}")
...
function IOSDispatcher:IAP_finish_transcation( )
	local json = string.format("{\"funcID\":\"iap_finish\"}")	
	IOSDispatcher:sync_send_to_oc( json )
end
...
function IOSDispatcher:appStoreUpdate( url)
	...
	local js = string.format("{\"funcID\":\"appstore_update\",\"updateUrl\":\"%s\"}",url)
    local ret = IOSDispatcher:async_send_to_oc( js ,update_callback)
    return ret
end
...
function IOSDispatcher:gamecenter_authenticateLocalUser( callback )
	...
	local json = string.format("{\"funcID\":\"gamecenter_authenticate\"}")
...
function IOSDispatcher:show_admob_banner(  )
	-- local did_recharge = CCUserDefault:sharedUserDefault():getStringForKey('did_recharge');
	-- if did_recharge == nil or did_recharge == ""  then
		local json = string.format("{\"funcID\":\"admob_banner\"}")
		IOSDispatcher:sync_send_to_oc(json)
	-- end
end
...
function IOSDispatcher:open_push_service(post_url )
	...
	local json = string.format("{\"funcID\":\"addPushService\"}")
...
function IOSDispatcher:showMsgBox( msg,title)
	...
	local js = string.format("{\"funcID\":\"showMessge\",\"msg\":\"%s\",\"title\":\"%s\"}",msg,title)

    IOSDispatcher:async_send_to_oc( js ,callback)
