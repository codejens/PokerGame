...
super_class.ShengJie(Window)

local _left_slotitem_item_source = nil   --右侧源item，保存起来，动态修改
local _left_slotitem_item_target = nil   --右侧目标item，保存起来，动态修改
local _left_slotitem_item_need = nil     --右侧需要升级道具item，保存起来，动态修改
...
function ShengJie:flash_all_item( )
    self:syn_sele_item()
    self:flash_all_item_right()
end

-- 刷新人物装备。获取最新人物身上物品后会被调用
function ShengJie:flash_all_item_char(  )
    self:syn_sele_item()
    self:flash_all_item_right()
end

function ShengJie:syn_date(  )
    self:syn_sele_item()
    self:flash_all_item_right()
end

-- function ShengJie:update_select_equip_list(  )
...
function ShengJie:update( update_type )
    --  print("ShengJie:update( update_type )",update_type)
    --背包 更新
...
function ShengJie:syn_sele_item(  )
...
-- 显示操作结果的提示
function ShengJie:show_handle_result( result, result_msg )
...
function  ShengJie:get_all_panel()
...
--================================================================================================================================
--创建右侧panel
function ShengJie:create_right_panel( pos_x, pos_y, width, height, texture_name )local panel  =  CCBasePanel:panelWithFile( pos_x, pos_y, width, height,texture_name , 500, 500 )
    --左底板
   ...
   -- _left_slotitem_item_need2   = self:create_item( panel, nil, 240, 140, 55, 55, "item_need2" )          --需要道具items
    ...
    -- 需要材料的数量
    local label_temp = UILabel:create_label_1("", CCSize(150,40), 108, 10, font_size, CCTextAlignmentCenter, 255, 255, 0)
    ...
    -- 升级费用
    local label_temp2 = UILabel:create_label_1("", CCSize(150,40), 122, 178, 16, CCTextAlignmentCenter, 255, 255, 0)
    ...
    --按钮显示的名称
    -- local name_image = CCZXImage:imageWithFile( 72, 30, 72, 29, UIPIC_FORGE_065 ); 
    -- name_image:setAnchorPoint(0.5,0.5)
    local name_text =UILabel:create_lable_2( Lang.forge.sj_and_tp[1], 0,0, 18, CCTextAlignmentCenter )
    ...
    local function help_btn_callback( eventType,x,y )
           -- if eventType == TOUCH_CLICK then 
            ForgeModel:show_help_panel( UILH_FORGE.shengjie_notic, Lang.forge.sjsm )
          -- end
        -- return true
    end
    ...
    local help_img = ZImage:create(help_btn, UILH_FORGE.shengjie_img, 0, 0, -1, -1 )
    ...
    --获取说明
    -- local explain_but = UIButton:create_button( 330, 442, 70, 20, UILH_FORGE.shengjie_img, UILH_FORGE.shengjie_img, UILH_FORGE.shengjie_img,999)
    ...
    -- local shengjie_img = ZImage:create(panel, UILH_FORGE.shengjie_img, 340, 450, -1, -1)
    -- panel:addChild( shengjie_img )
...
--更新右边的数据
function ShengJie:draw_mid_equip_attr( panel )
...
-- 给一个显示面板加购买按钮 
-- function ShengJie:add_buy_but_to_show_panel( slotitem_need)
--     --xiehande 通用按钮修改  UIPIC_FORGE_061 ->UIPIC_COMMOM_001
--     local but_1 = CCNGBtnMulTex:buttonWithFile( 260-5, 10, -1, -1, UIPIC_COMMOM_001, 500, 500)
...
-- 显示选中的装备的属性
function ShengJie:show_equi_attr( item_date )
    ...
    if ( _r_up_equi_or_bag == "equipment" ) then
        user_item = UserInfoModel:get_equi_by_id( _left_slotitem_item_target.item_series )
    elseif ( _r_up_equi_or_bag == "bag" ) then
        user_item = ItemModel:get_item_by_series( _left_slotitem_item_target.item_series )
    end
    local _user_item = {};
...
function ShengJie:create_item( panel, item_series, po_x , po_y, size_w, size_h , item_type)
...
-- 设置右侧，item的属性:类型 字符串（ 无用 ）,物品id(是基本属性，不是序列号，注意了)
function ShengJie:set_left_item( type, item_id, strong_level, item_series ,item_config_index)
    ...
    if ( item_series ) then
        ...
        local target_item_id, meta_item_id, mete_need_num,need_money = EquipEnhanceConfig:get_sj_info( item_config_index )
        local item = _left_slotitem_item_source                                   --左侧的源item
        item.item_series  = item_series                                           --记录序列号，发送请求的时候需要
        _left_slotitem_item_target.item_series = item_series                      --记录序列号，显示tip现在需要model
        item.item_id = item_id                                                    --记录物品id，发送请求的时候需要
        item.strong_level = strong_level                                          -- 强化等级
        -- item:set_icon( item_id )                                                  --设置图标
        item:set_color_frame( item_id, -2, -2, 72, 72 )                                           -- 左装备颜色框
        _left_slotitem_item_target:set_color_frame( target_item_id, -2, -2, 72, 72 )              -- 右装备颜色框
        ...
        _left_slotitem_item_target:set_item_quality( user_item.void_bytes_tab[1], pj +1 )
        ...
        -- local item_base = ItemConfig:get_item_by_id( item_id )                    -- 获取基础数据
        -- 根据套装id， 如果在定义的表中有该id对应的值，就获取第二个值（即升级辅助材料id），否则设置为默认icon (999999)
        -- local check_t = _suit_can_up_id_t[ item_base.suitId ]
        -- local meta_item_id = check_t and check_t[2] or 999999
        _left_slotitem_item_need.item_id = meta_item_id


        if item_base then
            item:set_item_quality( user_item.void_bytes_tab[1] ,pj) 
            item:set_icon( item_id )                                              --设置图标
        end
        ...
        if item_series ~= nil then
            atta_value_left = _static_quantity_color[item_base.color+1].."【".._static_quantity_str[ user_item.void_bytes_tab[1] ].."】";
            atta_value_right = _static_quantity_color[item_base2.color+1].."【".._static_quantity_str[ user_item.void_bytes_tab[1] ].."】";
        end
        ...
        local item = _left_slotitem_item_source  
...
-- 获取需要的升级辅助材料的数量
function ShengJie:get_mate_num( item_id )
    local count_ret = 0
    _mate_series_t = {}
...
function ShengJie:flash_all_label( )
    if _item_num_need ~= nil and _item_num_need ~= 0 then               --如果需要的数字为空或者0，则不显示
        self.label_t["need_name"]:setString(color_yellow .. ForgeModel:get_item_name_with_color(_left_slotitem_item_need.item_id))
        -- 如果材料不够，则显示红色
        local color_hex = nil
        ...
        self.label_t[ "need_money" ]:setString(color_yellow.._pay_for_synth);   -- [1005]="手续费:" -- [411]="仙币"
    else
...
-- 设置转移按钮是否可以有效
function ShengJie:check_action_but_able(  )
...
function ShengJie:check_can_upgrade( ... )
    if _item_num_need == nil or _item_num_need == 0 or _item_num_have < _item_num_need then 
...
-- 发送装备升级的消息
function ShengJie:send_request_upgrade(  )

   if _left_slotitem_item_source.item_series == nil then
        local show_content = Lang.forge.sj_and_tp[2]-- [145]="请先选择装备"
        GlobalFunc:create_screen_notic( show_content, nil, 250, 130 )
        return
    end
    ...
    local htype      = 13                                                     -- 操作类型,装备提品
    local param_count = 0
    local param_arr  = {  }                                    
    -- print("装备升阶...........................................",item_count, id_items, htype)
...
--================================================================================================================================
---创建左侧panel
function ShengJie:create_left_panel( pos_x, pos_y, width, height, texture_name )
...
--创建左侧物品栏的一个item 和 信息。因为和右侧物品处理方式不一样，要分开写一个方法。
...
function ShengJie:create_item_right( item_id, po_x , po_y, size_w, size_h , num, item_series ,item_config_index)
    require "config/ItemConfig"
    ...
    slotItem:setPosition( po_x , po_y )
    slotItem.item_config_index = item_config_index
    -- 设置图标的强化等级
    if num then
    ...
    --设置回调单击函数
    local function f1()
        self:set_left_item( "", item_id, num, item_series,slotItem.item_config_index)
        self:set_r_up_one_slot_disable( slotItem  )
    end
    slotItem.f1 = f1
    ...
    -- label_temp = UILabel:create_label_1("强化 +"..strong, dimensions, 110, 5, 16, CCTextAlignmentLeft, 255, 255, 0)
    ...
    label_temp = UILabel:create_lable_2( "【".._static_quantity_str[ user_item.void_bytes_tab[1] ].."】" , 75, 0, font_size+2, ALIGH_LEFT)
    slotItem.num_label  = label_temp               --存储起来，使用关键字获取并修改显示内容
    slotItem.view:addChild( label_temp )           --因为lable要随slot共存亡，所以是添加到slot的view中
...
function ShengJie:create_bag_slot( panel )
    ...
    for k,v  in pairs(check_info) do
        ...
        equip = v
        -- 获取装备的基础信息
        -- 判断该id是否可以提升品阶 先判断是否装备
        if ( _r_up_equi_or_bag == "bag" ) then 
            local item_base = ItemConfig:get_item_by_id( equip.item_id )
            if ( item_base.type >0 and item_base.type <11  ) then
                -- 遍历真装备
                local item_config_index = self:check_item_can_sj( equip.item_id ,equip.void_bytes_tab[1] )
                if ( item_config_index ) then 
                    slot_par[ #slot_par + 1] = { equip.item_id, 15, 15 , 55, 55, equip.strong, equip.series ,item_config_index }
                end
            end
        else
            -- 遍历真装备
            local item_config_index = self:check_item_can_sj( equip.item_id ,equip.void_bytes_tab[1])
            if ( item_config_index ) then
                slot_par[ #slot_par + 1] = { equip.item_id, 15, 20 , 55, 55, equip.strong, equip.series ,item_config_index }
            end
        end
    end
    row_index_count = 1
    ...
    local arrow_up = CCZXImage:imageWithFile(_t_scroll_item_info.x - 5 + _t_scroll_item_info.width ,
     _t_scroll_item_info.y - 5, 10, -1 , UILH_COMMON.scrollbar_down, 500, 500)
    local arrow_down = CCZXImage:imageWithFile(_t_scroll_item_info.x - 5 + _t_scroll_item_info.width,
      _t_scroll_item_info.y + _t_scroll_item_info.height-5, 10, -1, UILH_COMMON.scrollbar_up, 500 , 500)
...
-- 默认选中第一个
function ShengJie:set_default_item(  )
    -- 默认选中第一个
...
function ShengJie:create_scroll_area( panel_table_para ,pos_x, pos_y, size_w, size_h, colu_num, sight_num, bg_name)
        local row_num = math.ceil(#panel_table_para / colu_num)
    if row_num > 0 and row_num < 1 then
        row_num = 1
    end
...
-- 创建一个道具显示面板
function ShengJie:create_item_show_panel( panel_date, x, y, w, h )
    local t = panel_date                -- 历史原因，使用t
    local item_show_panel = {}
    local bg = CCBasePanel:panelWithFile( x, y , w, h, UILH_COMMON.bg_10, 500, 500 )
    item_show_panel.view = bg 
...
function ShengJie:check_create_slot_if_sele( slotItem )
...
-- 判断一个装备是否可以提阶
function ShengJie:check_item_can_sj( item_id ,item_quality)

    require "config/EquipEnhanceConfig"
    local upgrade_t = EquipEnhanceConfig:get_can_tp_items( )
    local player = EntityManager:get_player_avatar();
    local start_index = ( player.job-1 )*30
    -- 遍历本职业40-60级装备
    for i= start_index,start_index+30 do
        if ( upgrade_t[i] == item_id ) then
            return i;
        end
    end
    if ( item_quality > 3 ) then
        -- 遍历本职业40-60级的真装备
        for i= start_index + 120,start_index + 150 do
            if ( upgrade_t[i] == item_id ) then
                return i;
            end
        end
...
--刷新右侧所有item. 
function ShengJie:flash_all_item_right( )
    self:remove_all_item_right( )
    self:create_bag_slot( self.panel_t["right_panel"] )
end

--清空右侧面板所有item
function ShengJie:remove_all_item_right( )
...
-- -- 设置制定slot为无效效状态
function ShengJie:set_r_up_one_slot_disable( slotItem  )
    -- 必须先设置其他为有效装备
...
-- 设置材料栏某个item可以点击
function ShengJie:set_r_up_one_slot_able( item_series )
    for i, slot in pairs(self.right_item_t) do 
...
-- 设置锁
function ShengJie:set_slot_lock( item_series, slotItem )
...
-- 测试tips
function ShengJie:show_tool_tips_left(  type )
    local extend_date = {}
    if "source" == type then
...
function ShengJie:show_tool_tips_rihgt(  item_id )
    local extend_date = {}
    ToolTipMgr( "item", item_id,extend_date)
end

-- 设置按钮状态（实现单选），参数：位置标记，按钮序列号
function ShengJie:but_select_one( place, index )
    local but_table = {}
