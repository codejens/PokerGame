...
function SlotItem:__init( width, height )
	...
	-- self.bg_frame = CCBasePanel:panelWithFile(0, 0, -1, -1, "sui/common/slot_frame.png")
...
function SlotItem:set_lock( if_lock )
	...
	if if_lock then
		if self.lock_icon == nil then
			-- 0,0 ->2,2
            self.lock_icon = CCZXImage:imageWithFile(2, 6, -1, -1, UILH_NORMAL.item_lock)
...
function SlotItem:create_label_left_up(  )
	...
	--10-》12 self.height - 10 -self.height - 12
	self._label_left_up:setPosition(CCPointMake(12, self.height - 14))
	self._label_left_up:setScale(0.7)
