...
super_class.OtherEquipWin(NormalStyleWindow)
...
local _if_tianyuan = false              -- 是否是天元之城的城主
--
function OtherEquipWin:__init( window_name, texture_name,is_grid,width,height)
...
-- function OtherEquipWin:create_a_button(panel, pos_x, pos_y, size_w, size_h, image_n, image_s ,but_name, but_name_siz_w, but_name_siz_h, but_index)
...
--切换功能窗口:   1:装备   2：信息
function OtherEquipWin:change_page( but_index )
...
function OtherEquipWin:create( texture_name )
	return UserAttrWin( texture_name, 10, 46, 389, 429);
end

-- 供外部调用，刷新所有数据
function OtherEquipWin:update_win( update_type )
    require "UI/UIManager"
    local win = UIManager:find_visible_window("other_equip_win")
    if win then
        -- 把数据交给背包窗口显示
...
--刷新所有属性数据
function OtherEquipWin:update( update_type )
    if _if_show_other_player then
        return 
    end
...
-- function OtherEquipWin:show_body_level_panel( type )
...
-- 关闭全身属性面板
-- function OtherEquipWin:close_body_level_panel(  )
--     if self.has_show_star_or_sun then
...
-- 打开或者关闭是调用. 参数：是否激活
function OtherEquipWin:active( if_active )
    if if_active then
        self:update()
    else
...
function OtherEquipWin:open_other_panel( player_obj, equip_list, if_tianyuan )
    print("OtherEquipWin:open_other_panel!!!~~", player_obj.name)
    -- for k,v in pairs(player_obj) do
    --     print(k,v)
    -- end
    print("---------------------------------------------------------------")
    ...
    -- 连续打开窗口会有问题，所以先关掉旧窗口
    local old_win = UIManager:find_window("other_equip_win")
    if old_win then
        UIManager:destroy_window("other_equip_win")
    end

    _other_player = player_obj
    _other_equip_list = equip_list
    _if_tianyuan = if_tianyuan

    local win = UIManager:show_window( "other_equip_win" )
    _if_show_other_player = true
    win:change_page( 1 )
    win:set_detail_info_btn_visible(if_tianyuan)
    win.current_panel:show_detail_info_btn(false)
    --更改title圖片
    if win.window_title then
        win.window_title:setTexture(UILH_OTHER.show_other_title)
    end
    -- win:open_other_panel()
    -- if if_tianyuan then 
...
-- 是否显示的是天元之城城主的装备
function OtherEquipWin:set_detail_info_btn_visible(if_tianyuan)
    if if_tianyuan then 
        -- win.radio_but_t[2]:setIsVisible( false )
        if self.current_panel.show_detail_info_btn then
            self.current_panel:show_detail_info_btn(false)
        end
    else
        -- win.radio_but_t[2]:setIsVisible( true )
        if self.current_panel.show_detail_info_btn then
            self.current_panel:show_detail_info_btn(true)
        end
    end
end

-- 由于时间紧急，原谅我把model层代码写在这（OtherEquipWin之前没人去写model），note by guozhinan
function OtherEquipWin:open_other_attr_win()
    OtherAttrWin:open_other_panel(_other_player, _other_equip_list, _if_tianyuan)
end

function OtherEquipWin:get_other_player_equip_list()
    return _other_equip_list;
end

function OtherEquipWin:destroy()
    _if_show_other_player = false
    _if_tianyuan = false
    Window.destroy(self)
