...
super_class.HLDJieJiu(Window)

-- 初始化
function HLDJieJiu:__init( )

    -- 创建滑动条
    MUtils:create_zximg(self.view,UIPIC_GRID_nine_grid_bg3,12,12,366,378,500,500);
   	self:create_scroll_view();
    self.shuoming_lab = MUtils:create_ccdialogEx(self.view,"你暂时没有仙仆可以去解救。解救对象为自己仙宗身份为仙仆的玩家。",
                42,170,300,100,99,16)
    self.shuoming_lab:setIsVisible(false);
end


function HLDJieJiu:create_scroll_view()
...
function HLDJieJiu:create_scroll_item( parent,i )
    ...
    local function btn_fun( eventType,args,msgid )
        if eventType == TOUCH_CLICK then
            HuanLeDouCC:req_save_kugong( info.id )
            UIManager:hide_window("hld_main_win")
        end
        return true;
    end
    MUtils:create_btn_and_lab(parent,UIResourcePath.FileLocate.common.."button2.png",nil,btn_fun,265,14,-1,-1,LangGameString[1235],16); -- [1235]="解救"
...
function HLDJieJiu:active( show )
    if ( show ) then 
        HuanLeDouCC:req_can_save_list()
    end
end

function HLDJieJiu:update_scroll( data )
