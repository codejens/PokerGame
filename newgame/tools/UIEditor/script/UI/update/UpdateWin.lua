...
function UpdateWin:init( root )
    ...
    --     --必须火
        ...
        --全职忍者
        --ui2/update/background.wd
        _father_node = CCBasePanel:panelWithFile( 0, 0, _refWidth(1.0), _refHeight(1.0), "" )
    end
    
    root:addChild( _father_node )
    --UpdateWin:begin_bg_animation(  )
end

-- 版本信息显示
local _current_game_v_lable = nil         -- 当前游戏版本
...
function UpdateWin:show_version_info(text)
    if _current_game_v_lable == nil then
        panel =  _father_node
        self.begin_x = 16
...
function UpdateWin:setup_download_ui( )
    if _download_bar == nil then
        ...
        _download_per_lable = UpdateWin:create_lable_2( "0%", _barWidth * 0.5, 10, 16, ALIGN_LEFT )
        _download_bar.view:addChild( _download_per_lable )
        _download_per_lable:setAnchorPoint(CCPointMake(0.5,0.0))

        _bar_state_lable = UpdateWin:create_lable_2( LangCommonString[75], 70, 37, 16, ALIGN_LEFT ) -- [75]="#cffff00更新资源"
        ...
        _rate_lable = UpdateWin:create_lable_2( LangCommonString[77], _barWidth - 240, -27, 16, ALIGN_LEFT ) -- [77]="#cffff00速度:"
        _download_bar.view:addChild( _rate_lable )
        _download_rate = UpdateWin:create_lable_2( "0KB/S", _barWidth - 120, -27, 16, ALIGN_LEFT )
...
function UpdateWin:setUpdateCountText(message)
    if _download_count then 
        _download_count:setString(message)
    end
end
...
function UpdateWin:downloading( curr_num, max_num, speed )
    ...
    _download_per_lable:setString( down_percent_value.."%" )

    -- 下载的数值显示
    _download_count:setString( curr_num .. "/" .. max_num.."KB" )

    -- 下载速度
    _bar_state_lable:setIsVisible(true)
...
function UpdateWin:update_end(  )
    if _download_count then
        _download_count:setString( LangCommonString[78] ) -- [78]="启动游戏"
        --_download_rate:setString("0KB/S")
        _download_per_lable:setString( "100%" )
...
function UpdateWin:download_end(  )
    if _download_count then
        _download_count:setString( LangCommonString[79] ) -- [79]="下载成功"
        _download_rate:setString("0KB/S")
        _download_per_lable:setString( "100%" )
...
function UpdateWin:begin_release(  )
    _bar_state_lable:setString( LangCommonString[80] ) -- [80]="#cffff00解压资源"
end
...
function UpdateWin:clear_win_but_bg(  )
    if _download_bar then
        ...
        --_father_node:removeChild( _current_game_v_lable, true )
        _father_node:removeChild( _update_info_panel, true )
...
-- ===========================================
function UpdateWin:create_progress_bar( x, y, w, h, image_bg, image_front, max_value, font_info, margin_t, if_show_value )
