...
--创建超级团购活动页面类
super_class.LDGroupBuyPage(GotoActivityPage)
...
--作者：肖进超
--时间：2014.10.29
function LDGroupBuyPage:__init()
	--创建私有变量
...
-- 团购自定义界面排版
function LDGroupBuyPage:create_panel_ud( panel )
	-- body
	...
	-- 展示item
	local items_conf = SuperGroupBuyConfig:getConfigNewYear(CommonActivityConfig.NewLonelyDay)
	for i=1, 2 do
...
-- 团购自定义界面排版item
function LDGroupBuyPage:create_slot_items(giftIndex, items_conf)
	...
	for i=2, 4 do
		local item_lttl = MUtils:create_slot_item2( items_bg, UILH_COMMON.slot_bg, 28+80*(i-2), 5, 67, 67, items_conf[i].id, nil, 6)
...
--作者：肖进超
--时间：2014.10.29
function LDGroupBuyPage:destroy()
	--获取父类
	local t_pageParent = LDGroupBuyPage.super
	--调用父类的析构函数
...
--作者：肖进超
--时间：2014.10.29
function LDGroupBuyPage:showPageAction()
	LonelyDayModel:showLDGroupBuyPage(self._isInit)
	--请求团购活动信息
...
--作者：肖进超
--时间：2014.10.29
function LDGroupBuyPage:forwardButtonAction()
	LonelyDayModel:openGroupBuyActivityWin()
end
...
--作者：肖进超
--时间：2014.10.29
-- function LDGroupBuyPage:czForwardButtonAction()
-- 	LonelyDayModel:openGroupBuyActivityWin()
-- end

function LDGroupBuyPage:setMyPoint( point )
