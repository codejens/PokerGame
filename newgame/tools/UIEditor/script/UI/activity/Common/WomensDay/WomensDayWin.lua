...
require "UI/activity/Common/WomensDay/WDDailyQuotaBuyPage"              --每日限购
...
require "UI/activity/Common/WomensDay/WDVLuopanPage"  -- 幸运转盘
-- require "UI/activity/Common/ValentineDay/CVEveryConsumePage"                
--定义情人节窗口类
super_class.WomensDayWin(CommonActivityBaseWin)

-- 如果要调转两个页面位置，需要修改：1.WomensDayWin的self._pageClassGroup表顺序。2.ValentineDayModel的页面序号。3.ValentineDayParam中参数顺序 by gzn
...
--作者：陈亮
--时间：2014.10.27
function WomensDayWin:destroy()
	--获取父类
	local t_pageParent = WomensDayWin.super
	--调用父类的析构函数
...
--作者：陈亮
--时间：2014.10.27
function WomensDayWin:showWinAction()
	WomensDayModel:showWomensDayWin(self._isInit)
end