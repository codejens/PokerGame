...
--创建超级团购活动页面类
super_class.WhiteGroupBuyPage(GotoActivityPage)
...
--作者：肖进超
--时间：2014.10.29
function WhiteGroupBuyPage:__init()
	--创建私有变量
...
-- 团购自定义界面排版
function WhiteGroupBuyPage:create_panel_ud( panel )
	-- body
	...
	-- 展示item
	local items_conf = SuperGroupBuyConfig:getConfigNewYear(CommonActivityConfig.ValentineWhiteDay)
	for i=1, 2 do
...
-- 团购自定义界面排版item
function WhiteGroupBuyPage:create_slot_items(giftIndex, items_conf)
...
--作者：肖进超
--时间：2014.10.29
function WhiteGroupBuyPage:destroy()
	--获取父类
	local t_pageParent = WhiteGroupBuyPage.super
	--调用父类的析构函数
...
--作者：肖进超
--时间：2014.10.29
function WhiteGroupBuyPage:showPageAction()
	ValentineWhiteDayModel:showLDGroupBuyPage(self._isInit)
	--请求团购活动信息
...
--作者：肖进超
--时间：2014.10.29
function WhiteGroupBuyPage:forwardButtonAction()
	ValentineWhiteDayModel:openGroupBuyActivityWin()
end

function WhiteGroupBuyPage:setMyPoint( point )
