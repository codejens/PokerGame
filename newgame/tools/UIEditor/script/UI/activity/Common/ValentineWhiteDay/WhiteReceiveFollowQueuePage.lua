--CWhiteReceiveFollowQueuePage.lua
--内容：收花排行榜活动页面类
--作者：guozhinan
--时间：2015.2.7
...
--创建鲜花排行榜活动页面类
super_class.WhiteReceiveFollowQueuePage(NormalInfoPage)
...
function WhiteReceiveFollowQueuePage:__init()
...
function WhiteReceiveFollowQueuePage:refreshData()
	local receive_flower_info = BAReceiveFlowerModel:get_receive_flower_info()
	-- 刷新送花数量
...
--功能：设置滑动框标题组
function WhiteReceiveFollowQueuePage:setItemTitleGroup(itemTitleGroup)
	self._itemTitleGroup = itemTitleGroup
end

--功能：设置滑动框奖励组
function WhiteReceiveFollowQueuePage:setAwardDataGroup(awardDataGroup)
	self._awardDataGroup = awardDataGroup
end

--功能：重置滑动框的内容
function WhiteReceiveFollowQueuePage:resetScroll()
...
--功能：页面析构函数
function WhiteReceiveFollowQueuePage:destroy()
	-- self.get_award_btn_table = {}
	--获取父类
	local t_pageParent = WhiteReceiveFollowQueuePage.super
	--调用父类的析构函数
...
function WhiteReceiveFollowQueuePage:showPageAction()
	ValentineWhiteDayModel:showReceiveFollowQueuePage(self._isInit)
	-- 请求新数据
...
--作者：陈亮
--时间：2014.10.27
function WhiteReceiveFollowQueuePage:gotoQueue()
	ValentineWhiteDayModel:gotoReceiveFollowQueueWin(9)
end