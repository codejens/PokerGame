...
--创建超级团购活动页面类
super_class.YXGroupBuyPage(GotoActivityPage)
...
--作者：肖进超
--时间：2014.10.29
function YXGroupBuyPage:__init()
	--创建私有变量
...
-- 团购自定义界面排版
function YXGroupBuyPage:create_panel_ud( panel )
	-- body
	...
	-- 展示item
	local items_conf = SuperGroupBuyConfig:getConfigNewYear(CommonActivityConfig.LanternDay)
	for i=1, 2 do
...
-- 团购自定义界面排版item
function YXGroupBuyPage:create_slot_items(giftIndex, items_conf)
...
--作者：肖进超
--时间：2014.10.29
function YXGroupBuyPage:destroy()
	--获取父类
	local t_pageParent = YXGroupBuyPage.super
	--调用父类的析构函数
...
--作者：肖进超
--时间：2014.10.29
function YXGroupBuyPage:showPageAction()
	LanternDayModel:showYXGroupBuyPage(self._isInit)
	--请求团购活动信息
...
--作者：肖进超
--时间：2014.10.29
function YXGroupBuyPage:forwardButtonAction()
	LanternDayModel:openGroupBuyActivityWin()
end
...
--作者：肖进超
--时间：2014.10.29
-- function YXGroupBuyPage:czForwardButtonAction()
-- 	LonelyDayModel:openGroupBuyActivityWin()
-- end

function YXGroupBuyPage:setMyPoint( point )
