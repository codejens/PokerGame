-- CreateTeamDialog.lua
-- created by guozhinan on 2015/4/23
-- 创建队伍时选择队伍类型的对话框

super_class.CreateTeamDialog(Window)

local _cb_fun = nil;

function CreateTeamDialog:show(cb_fun)
    -- 阻止连续打开行为
    local win = UIManager:find_visible_window("create_team_dialog")
    if win then
        return;
    end

    local win = UIManager:show_window("create_team_dialog",true);
    if win then
        _cb_fun = cb_fun;
    end
end
...
-- 
function CreateTeamDialog:__init( window_name, texture_name, is_grid, width, height,title_text )
    ...
    self.title_label = MUtils:create_zxfont(subtitle_bg,Lang.team[25],(width-6)/2,12,2,16);-- [25] = "创建队伍",

    MUtils:create_zxfont(panel,Lang.team[27],19,255,1,16);  -- [27] = "目标副本",

    self.selected_state_table = {false,false,false,false,true}  -- 默认选择”无“副本类型
    self.switch_but_table = {}
    self.switch_but_title = {Lang.team[20],Lang.team[21],Lang.team[22],Lang.team[23],Lang.team[24]}
    self.is_only_choice = true;     -- 标记是否一次只能选择一个
    local begin_x = 40;
    local top_y = 250;
    ...
    for i=1,5 do
        ...
        local function switch_button_func( is_selected )
            ...
            if is_selected == true and self.is_only_choice == true then
                for k=1,5 do
                    if i ~= k then
                        self.switch_but_table[k].set_state(false)
                        self.selected_state_table[k] = false;
                    end
                end
            end
        end
    ...
    local function btn_ok_fun(eventType,x,y)
        if eventType == TOUCH_CLICK then
            ...
            if has_choice == true then
                ...
                UIManager:destroy_window("create_team_dialog");
            else
                GlobalFunc:create_screen_notic(Lang.team[30]);  --"请先选择副本!"
            end
        end
        return true
    end
...
function CreateTeamDialog:active( show )

end

function CreateTeamDialog:destroy(  )
    _cb_fun = nil;
    Window.destroy(self)
end