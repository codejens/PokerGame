...
local _title_info							= {width = 275, height = 10, size = { 205, 60, 85 }, myenemy = Lang.friend.ememy[1], kill = Lang.friend.ememy[2], bekill = Lang.friend.ememy[3]} -- [1]="我的仇人(0/0)" -- [2]="击杀" -- [3]="被击杀"
local _scroll_info							= {height = 315, page_num = 10, maxnum = 1, gapSize = 10, scrollType = TYPE_HORIZONTAL_EX, scrollRunType = RUN_TYPE_PAGE}
local _edit_info							= {x = 44, y = 43, width = 250, height = 45, maxnum = 100, image =UILH_COMMON.bg_02, default_text_info = Lang.friend.common[1]} -- [1]="点击输入玩家名称"
  --xiehande   UILH_FRIEND010 ->UIPIC_COMMOM_002
local _add_enemy_info						= {width = -1, height = -1, image = {UILH_COMMON.btn4_nor, UILH_COMMON.btn4_nor}, text = Lang.friend.ememy[4]} -- [4]="添加仇人"
...
function Enemy:createEnemy(fatherPanel, width, height)
	...
	local killLabel = ZLabel:create(nil, _title_info.kill, 25, 0)
	local kill_label_size = killLabel:getSize()
	local killImage = ZImage:create(nil, UILH_NORMAL.up_b, kill_label_size.width+25, 0, -1, -1)
	local kill_image_size = killImage:getSize()
	local kill_size_width = kill_label_size.width + kill_image_size.width
	local kill_size_height = kill_label_size.height
	if kill_size_height < kill_image_size.height then
		kill_size_height = kill_image_size.height
	end
	local kill = ZBasePanel:create( nil, nil, 0, 0, kill_size_width, kill_size_height )
	kill:addChild(killImage)
	kill:addChild(killLabel)
	local bekillLabel = ZLabel:create(nil, _title_info.bekill, 0, 0)
	local bekill_label_size = bekillLabel:getSize()
	local bekillImage = ZImage:create(nil, UILH_NORMAL.up_b, bekill_label_size.width, 0, -1, -1)
	local bekill_image_size = bekillImage:getSize()
	local bekill_size_width = bekill_label_size.width + bekill_image_size.width
	local bekill_size_height = bekill_label_size.height
	if bekill_size_height < bekill_image_size.height then
		bekill_size_height = bekill_image_size.height
	end
	local bekill = ZBasePanel:create( nil, nil, 0, 0, bekill_size_width, bekill_size_height)
	bekill:addChild(bekillImage)
	bekill:addChild(bekillLabel)
	self.title:addItem(enemyLabel)
	self.title:addItem(bekill)
	self.title:addItem(kill)
	-----------
	...
	self.scroll:setScrollCreatFunction(FriendModel.enemy_scroll_create_function)
	-----------
	-----------
	...
	-----------
	--添加仇人按钮
	self.add_enemy_btn = ZTextButton:create( nil, _add_enemy_info.text, _add_enemy_info.image, nil, _edit_info.x + _edit_info.width-2, 41, _add_enemy_info.width, _add_enemy_info.height, nil, _window_size.width, _window_size.width)
	self.add_enemy_btn:setTouchClickFun(FriendModel.enemy_add_enemy_btn_click_fun)
	-----------
	...
	-----------
	--标题背景图
	self.title_bg=CCZXImage:imageWithFile(14,429,width-12,33,UILH_NORMAL.title_bg4,500,500)
	...
	self.view:addChild(self.add_enemy_btn.view)
...
-----------
-----------
function Enemy:__init(windowName, texture_name, is_grid, width, height)
	self:createEnemy(nil, width, height)
end
-----------
function Enemy:clear_scroll_item()
	self.scroll:clear()
end
-----------
function Enemy:refresh_scroll_item()
	self:update_title_info()
	self.scroll:refresh()
end
-----------
function Enemy:get_scroll_info()
...
-- -----------
-- function Enemy:update_tip_info()
-- 	local item = self.title:getIndexItem(1)
-- 	if item ~= nil then
-- 		local curOnlineNum = FriendModel:get_enemy_online_num()
-- 		local curMaxNum = FriendModel:get_enemy_info_num()
-- 		item:setText(string.format("我的仇人(%d/%d)",curOnlineNum,curMaxNum))
-- 	end
-- end
----------
function Enemy:update_title_info()
	if self.title ~= nil then
		local templabel = self.title:getIndexItem(1)
		--require "model/FriendModel/FriendModel"
		local online_num = FriendModel:get_enemy_online_num()
		local max_num = FriendModel:get_enemy_info_num()
		templabel:setText( string.format(Lang.friend.ememy[5],online_num, max_num) ) -- [5]="我的仇人(%d/%d)"
	end
end
