...
local show_key = {"posx","posy","toSceneid","toPosx","toPosy","modelid"}
local _attr = {}
...
function MapTpEdite:create(  )
	...
	local function create_scroll(index )
		local panel = MapTpEdite:create_scroll( index )
		return panel
	end
...
function MapTpEdite:update_info( index )
	local date = MapModel:get_tp_date( )
	date = date[index]
	for i=1,#show_key do
...
function MapTpEdite:create_scroll( index )

	    local layout = {}
	    local npcdate = MapModel:get_tp_date( )
		...
		local function panel_func( eventType )
			 if eventType == TOUCH_CLICK then 
	            local text = label:getText()
			   	...
			   	_old_item = layout
			   	MapTpEdite:save_last_attr(  )
			   	_curr_select = index + 1
			   	MapTpEdite:update_info( _curr_select )
			   	MapTpEdite:move_camera_tile( _curr_select )
			   	return true
...
function MapTpEdite:move_camera_tile( index )
	local npcdate = MapModel:get_tp_date( )
...
function MapTpEdite:save_last_attr(  )
	if _curr_select then
		...
		MapModel:sava_one_tp_attr( _curr_select,attr_t )
	end
end

function MapTpEdite:get_curr_select( ... )
	return _curr_select
end

function MapTpEdite:update_tp( date )
	...
	for i=1,#date do
		...
		_npc_model[i] =ZXEntityMgr:sharedManager():createEntity(1, 9, x, y, body, 1, 1) 
		local body_path = string.format('frame/monster/%d',body)
		_npc_model[i]:changeBody(body_path)
	end
